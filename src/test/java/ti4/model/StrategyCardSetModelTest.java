...
public class StrategyCardSetModelTest extends BaseTi4Test {
    ...
    public void testSCSetModels() {
        for (StrategyCardSetModel scSetModel : Mapper.getStrategyCardSets().values()) {
            ...
            assertTrue(
                    validateSCIDs(scSetModel),
                    scSetModel.getAlias() + " [" + scSetModel.getName() + "]: Invalid SC IDs");
            assertTrue(
                    validateNoDuplicateInitiatives(scSetModel),
                    scSetModel.getAlias() + " [" + scSetModel.getName() + "]: Duplicate Initiative Values");
        }
    }

    private static boolean validateSCIDs(StrategyCardSetModel scSetModel) {
        if (Mapper.getStrategyCards().keySet().containsAll(scSetModel.getScIDs())) return true;
        System.out.println("SCSet **" + scSetModel.getName() + "** failed validation due to invalid SC IDs: `"
                + scSetModel.getScIDs() + "`");
        return false;
    }
