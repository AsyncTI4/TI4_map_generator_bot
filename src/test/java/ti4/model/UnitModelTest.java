...
public class UnitModelTest extends BaseTi4Test {
    ...
    private static boolean validateAsyncID(UnitModel unitModel) {
        if (Stream.of(UnitType.values()).anyMatch(type -> type.value.equalsIgnoreCase(unitModel.getAsyncId()))) return true;
        System.out.println("[TEST FAILURE] Unit **" + unitModel.getId() + "** failed validation due to invalid AsyncID: `" + unitModel.getAsyncId() + "`");
        return false;
    }
}
