...
class TransactionHelperTest extends BaseTi4Test {
    ...
    @Test
    void sendOfferIncludesPillageNoticeWhenSinglePillagerCanPillage() {
        SendOfferHarness harness = createHarness();
        Player mentak = addPlayer(harness.game, "pillager-1", "Mentak", "mentak", "orange");
        harness.receiverNeighbors.add(mentak);
        ...
        assertThat(messages.buttonMessagesFor(harness.receiverThread))
                ...
                .satisfies(message -> assertThat(message)
                        ...
                        .contains(harness.receiverOfferHeader())
                        .contains(harness.notice()));
        assertThat(messages.allText())
                .anyMatch(message -> message.contains("NOTICE OF REDISTRIBUTION"))
                .allMatch(message -> !message.contains("NOTICE OF PROXIMITY SURCHARGE"));
        assertThat(messages.publicText()).contains(harness.notice());
    }

    @Test
    void sendOfferIncludesEachPillagerWhenMultiplePillagersCanPillage() {
        SendOfferHarness harness = createHarness();
        Player mentak = addPlayer(harness.game, "pillager-1", "Mentak", "mentak", "orange");
        Player secondPillager = addPlayer(harness.game, "pillager-2", "Second Pillager", "saar", "purple");
        secondPillager.addAbility("pillage");
        harness.receiverNeighbors.add(mentak);
        harness.receiverNeighbors.add(secondPillager);

        SentMessages messages = invokeSendOffer(harness);

        assertThat(messages.publicText())
                .contains("NOTICE OF REDISTRIBUTION\nFrom: " + harness.receiver.getRepresentationNoPing() + "\nTo: "
                        + mentak.getRepresentationNoPing())
                .contains("NOTICE OF REDISTRIBUTION\nFrom: " + harness.receiver.getRepresentationNoPing() + "\nTo: "
                        + secondPillager.getRepresentationNoPing());
        assertThat(messages.buttonMessagesFor(harness.receiverThread))
                .singleElement()
                .satisfies(message -> assertThat(message)
                        .startsWith(harness.receiverThreadPrefix())
                        .contains(harness.senderOfferLine())
                        .contains(harness.receiverOfferHeader())
                        .contains(mentak.getRepresentationNoPing())
                        .contains(secondPillager.getRepresentationNoPing()));
    }
    ...
    private record SendOfferHarness(
            ...
            Player sender,
            Player receiver,
            LinkedHashSet<Player> receiverNeighbors,
