...
class PeekAtSecretDeck extends GameStateSubcommand {
    ...
    public PeekAtSecretDeck() {
        super("peek_secret_objective_deck", "Peek at the top secret objective deck", true, true);
        addOptions(new OptionData(OptionType.INTEGER, Constants.COUNT, "Number of secrets to peek at, default 1").setRequired(true));
        addOptions(new OptionData(OptionType.INTEGER, "index", "Which of the secrets to draw (1, 2, 3, ...)"));
    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        Integer peekedSoIndex = event.getOption("index", null, OptionMapping::getAsInt);
        if (peekedSoIndex == null) {
            MessageHelper.sendMessageToChannel(event.getChannel(), "To draw the peeked card, use the draw option and the index of the card (1, 2, 3; "
                + "from top to bottom).");
            return;
        }
        peekedSoIndex -= 1;
        if (peekedSoIndex < 0) {
            ...
            return;
        } else if (peekedSoIndex >= peekedObjectives.size()) {
        ...
        var secretId = peekedObjectives.get(peekedSoIndex);
