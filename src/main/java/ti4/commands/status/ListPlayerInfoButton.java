        ...
        switch (objID) {
            ...
            case "outer_rim", "control_borderlands" -> {
                ...
                for (Tile tile : game.getTileMap().values()) {
                    if (FoWHelper.playerHasUnitsInSystem(player, tile) && tile.isEdgeOfBoard(game)
                        && tile != FoWHelper.getPlayerHS(game, player)) {
                        edge++;
                    }
                }
                return edge;
            ...
            case "infrastructure", "protect_border" -> {
                ...
                for (String planet : player.getPlanets()) {
                    ...
                    if (uH != null && game.getTileFromPlanet(planet) != FoWHelper.getPlayerHS(game, player)
            ...
            case "conquer" -> {
                ...
                for (String planet : player.getPlanets()) {
                    Tile tile = game.getTileFromPlanet(planet);
                    if (tile != null && tile.isHomeSystem() && tile != FoWHelper.getPlayerHS(game, player)) {
                        count++;
                    }
                }
                return count;
            ...
            case "supremacy" -> {
                ...
                for (Tile tile : ButtonHelper.getTilesOfPlayersSpecificUnits(game, player, UnitType.Flagship,
                    ...
                    if ((tile.isHomeSystem() && tile != FoWHelper.getPlayerHS(game, player))
            ...
            case "distant_lands" -> {
                ...
                for (Player p2 : game.getRealPlayers()) {
                    ...
                    Tile tile = FoWHelper.getPlayerHS(game, p2);
                    if (tile == null) {
                        continue;
                    }
