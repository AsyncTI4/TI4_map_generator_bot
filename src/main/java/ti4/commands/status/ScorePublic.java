            ...
            for (Player p2 : player.getNeighbouringPlayers()) {
                if (p2.hasLeaderUnlocked("syndicatecommander")) {
                    ...
                    String msg = p2.getRepresentation(true, true)
                        + " you gained 1 trade goods due to your neighbor scoring a public objective while you have Fillipo Rois, the Tnelis commander. Your trade goods went from "
                        + (p2.getTg() - 1) + " -> " + p2.getTg();
    ...
    public static void informAboutScoring(GenericInteractionCreateEvent event, MessageChannel channel, Game game,
        ...
        if (poName.contains("Negotiate Trade Routes")) {
            ...
            if (oldtg >= 5) {
                ...
                MessageHelper.sendMessageToChannel(player.getCorrectChannel(),
                    player.getRepresentation() + " Automatically deducted 5 trade goods (" + oldtg + "->" + player.getTg()
                        + ").");
            } else {
                MessageHelper.sendMessageToChannel(player.getCorrectChannel(),
        ...
        if (poName.contains("Centralize Galactic Trade")) {
            ...
            if (oldtg >= 10) {
                ...
                MessageHelper.sendMessageToChannel(player.getCorrectChannel(),
                    player.getRepresentation() + " Automatically deducted 10 trade goods (" + oldtg + "->" + player.getTg()
                        + ").");
            } else {
                MessageHelper.sendMessageToChannel(player.getCorrectChannel(),
        ...
        if (poName.contains("Lead From the Front")) {
            ...
            if (currentStrat + currentTact >= 3) {
                if (currentStrat >= 3) {
                    ...
                    MessageHelper.sendMessageToChannel(player.getCorrectChannel(), player.getRepresentation() + " Automatically deducted 3 command tokens from your strategy pool (" + currentStrat + "->" + player.getStrategicCC() + ").");
                } else {
                    ...
                    MessageHelper.sendMessageToChannel(player.getCorrectChannel(), player.getRepresentation() + " Automatically deducted 3 command tokens from your tactic and strategy pools (" + currentCC + "->" + player.getCCRepresentation() + ").");
                }
            } else {
                MessageHelper.sendMessageToChannel(player.getCorrectChannel(),
                    "Did not deduct 3 command tokens because you didn't have that in your tactic and/or strategy pools.");
            }
        }
        if (poName.contains("Galvanize the People")) {
            ...
            if (currentStrat + currentTact >= 6) {
                if (currentStrat >= 6) {
                    ...
                    MessageHelper.sendMessageToChannel(player.getCorrectChannel(), player.getRepresentation() + " Automatically deducted 6 command tokens from your strategy pool (" + currentStrat + "->" + player.getStrategicCC() + ").");
                } else {
                    String currentCC = player.getCCRepresentation();
                    ...
                    player.setStrategicCC(0);
                    player.setTacticalCC(currentTact - subtract);
                    MessageHelper.sendMessageToChannel(player.getCorrectChannel(), player.getRepresentation() + " Automatically deducted 6 command tokens from your tactic and strategy pools (" + currentCC + "->" + player.getCCRepresentation() + ").");
                }
            } else {
                MessageHelper.sendMessageToChannel(player.getCorrectChannel(),
                    "Did not deduct 6 command tokens because you didn't have that in your tactic and/or strategy pools.");
            }
        }
    }
