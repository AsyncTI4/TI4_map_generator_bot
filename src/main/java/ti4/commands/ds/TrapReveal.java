                ...
                if (planetName.equals(planet) || planet == null) {
                    ...
                    if (reveal && planet != null) {
                        ...
                        if ("Minefields".equalsIgnoreCase(trapName)) {
                            ...
                            MessageHelper.sendMessageToChannel(ButtonHelper.getCorrectChannel(player, game), "Destroyed up to 2 enemy infantry from " + representation);
                        }
                        if ("Account Siphon".equalsIgnoreCase(trapName)) {
                            for (Player p2 : game.getRealPlayers()) {
                                ...
                                if (p2.getPlanets().contains(planet)) {
                                    ...
                                    MessageHelper.sendMessageToChannelWithButtons(ButtonHelper.getCorrectChannel(player, game), player.getRepresentation(true, true) + " use buttons to resolve",
                                        buttons);
                                }
                            }
                        }
