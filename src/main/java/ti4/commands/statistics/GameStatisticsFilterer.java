...
public class GameStatisticsFilterer {
    ...
    public static List<OptionData> gameStatsFilters() {
        ...
        filters.add(new OptionData(
                OptionType.STRING,
                GameStatisticsFilterer.GAME_TYPE_FILTER,
                "Filter games by game type, e.g. base, pok, absol, ds, action_deck_2, little_omega"));
        filters.add(new OptionData(
    ...
    private static boolean filterOnWinningFaction(String winningFactionFilter, Game game) {
        ...
        return game.getWinner().isPresent()
                && game.getWinner().get().getFaction().equals(winningFactionFilter);
    }
