...
public class ListMyGames extends HelpSubcommandData {
    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        Comparator<Game> mapSort = (map1, map2) -> map1.getName().compareTo(map2.getName());
        ...
        for (Game playerGame : games) {
            sb.append("`").append(Helper.leftpad("" + index, 2)).append(".`").append(getPlayerMapListRepresentation(playerGame, userID, showAverageTurnTime)).append("\n");
            index++;
        }
        MessageHelper.sendMessageToThread(event.getChannel(), user.getName() + "'s Game List", sb.toString());
    }

    private String getPlayerMapListRepresentation(Game playerGame, String userID, boolean showAverageTurnTime) {
