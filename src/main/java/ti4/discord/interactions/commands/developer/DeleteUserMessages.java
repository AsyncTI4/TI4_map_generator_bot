...
class DeleteUserMessages extends Subcommand {
    ...
    private static final int MAX_HISTORY_SCAN = 5_000;
    ...
    DeleteUserMessages() {
        ...
        addOptions(
                ...
                new OptionData(OptionType.CHANNEL, Constants.CHANNEL, "Channel to clean (defaults to current channel)"));
    }
    ...
    private static MessageChannel resolveTargetChannel(SlashCommandInteractionEvent event) {
        ...
        ChannelUnion channel = channelOption.getAsChannel();
    ...
    private static List<Message> findMostRecentMessagesByUser(MessageChannel channel, String userId, int count) {
        ...
        List<Message> batch = channel.getHistory().retrievePast(HISTORY_BATCH_SIZE).complete();
        int scanned = 0;
