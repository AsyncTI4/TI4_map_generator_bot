...
class GalacticEventsSetup extends GameStateSubcommand {
    ...
    GalacticEventsSetup() {
        ...
        addOptions(new OptionData(
                OptionType.BOOLEAN,
                Constants.AGE_OF_COMMERCE_MODE,
                "True to enable the Age of Commerce, per Codex 4."));
        addOptions(
                new OptionData(OptionType.BOOLEAN, Constants.TOTAL_WAR_MODE, "True to enable Total War, per Codex 4."));
        addOptions(new OptionData(
                OptionType.BOOLEAN,
                Constants.AGE_OF_FIGHTERS_MODE,
                "True to enable the Age of Fighters, per Thunders Edge."));
        addOptions(new OptionData(
                OptionType.BOOLEAN,
                Constants.CIVILIZED_SOCIETY_MODE,
                "True to enable Civilized Society, per Thunders Edge."));
        addOptions(new OptionData(
                OptionType.BOOLEAN,
                Constants.STELLAR_ATOMICS_MODE,
                "True to enable the Stellar Atomics, per Thunders Edge."));
        addOptions(new OptionData(
                OptionType.BOOLEAN,
    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        Boolean civilizedSocietyMode =
                event.getOption(Constants.CIVILIZED_SOCIETY_MODE, null, OptionMapping::getAsBoolean);
