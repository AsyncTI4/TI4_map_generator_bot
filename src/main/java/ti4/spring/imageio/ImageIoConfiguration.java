...
public class ImageIoConfiguration {
  ...
  private static boolean ensureWebpWriterRegistered() {
    ...
    try {
      ...
      ImageWriterSpi spi = (ImageWriterSpi) spiKlass.getDeclaredConstructor().newInstance();
  ...
  private static boolean hasWebpWriter() {
    boolean hasWebpWriterQuickCheck = Arrays.stream(ImageIO.getWriterFormatNames())
                          .anyMatch("webp"::equalsIgnoreCase);
