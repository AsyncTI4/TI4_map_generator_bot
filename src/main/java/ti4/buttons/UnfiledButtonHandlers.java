 ...
 */
public
class UnfiledButtonHandlers { // TODO: move all of these methods to a better location, closer to the original button
    // call and/or other related code
    ...
    public static void genericRemove(ButtonInteractionEvent event, Player player, String buttonID, Game game) {
        ...
        MessageHelper.sendMessageToChannel(
                event.getMessageChannel(),
                "Chose to remove units from " + game.getTileByPosition(pos).getRepresentationForButtons(game, player));
        MessageHelper.sendMessageToChannelWithButtons(
                event.getMessageChannel(), "Use buttons to select the units you wish to remove.", systemButtons);
        ButtonHelper.deleteMessage(event);
    }
    ...
    public static void getACFrom(ButtonInteractionEvent event, Player player, String buttonID, Game game) {
        ...
        MessageHelper.sendMessageToChannelWithButtons(
                ...
                player.getRepresentationUnfogged() + ", select which action card you wish to steal.",
                buttons);
        ButtonHelper.deleteMessage(event);
    }
    ...
    public static void miltyFactionInfo(Player player, String buttonID, Game game) {
        ...
        switch (remainOrPicked) {
            ...
            case "remaining" -> displayFactions.addAll(
                    game.getMiltyDraftManager().remainingFactions());
        }
    ...
    public static void removeAllStructures(ButtonInteractionEvent event, Player player, String buttonID, Game game) {
        ...
        MessageHelper.sendMessageToChannel(
                event.getMessageChannel(), "Removed all units on " + planet + " for " + player.getRepresentation());
    ...
    public static void arboAgentOn(ButtonInteractionEvent event, Player player, String buttonID, Game game) {
        ...
        MessageHelper.sendMessageToChannelWithButtons(
                event.getChannel(),
                player.getRepresentationUnfogged() + " select which unit you'd like to place down",
                buttons);
        ButtonHelper.deleteMessage(event);
    }
    ...
    public static void useTech(ButtonInteractionEvent event, Player player, String buttonID, Game game) {
        ...
        if (!tech.equalsIgnoreCase("st")) {
            String useMessage =
                    player.getRepresentation() + " used technology: _" + techModel.getRepresentation(false) + "_.";
    ...
    public static void reduceComm(ButtonInteractionEvent event, Player player, String buttonID, Game game) {
        ...
        String message = player.getFactionEmojiOrColor() + " reduced commodities by " + tgLoss + " ("
                + player.getCommodities() + "->" + (player.getCommodities() - tgLoss) + ")";
    ...
    public static void increaseTGonSC(ButtonInteractionEvent event, String buttonID, Game game) {
        ...
        MessageHelper.sendMessageToChannel(
                ...
                "Added 1 trade good to " + Helper.getSCName(scNum, game) + ". There " + (tgCount == 0 ? "is" : "are")
                        + " now " + (tgCount + 1) + " trade good" + (tgCount == 0 ? "" : "s") + " on it.");
    }
    ...
    public static void cancelSpaceHits(ButtonInteractionEvent event, Player player, String buttonID, Game game) {
        ...
        String msg2 =
                player.getRepresentationNoPing() + ", you may automatically assign " + (h == 1 ? "the hit" : "hits")
    ...
    public static void movedNExplored(
        ...
        if (dsdihmy) {
            ...
            MessageHelper.sendMessageToChannel(mainGameChannel, info[1] + " was exhausted by _Impressment Programs_.");
        }
    ...
    public static void revealPOStage(ButtonInteractionEvent event, String buttonID, Game game) {
        ...
        if (!game.isRedTapeMode()) {
            ...
            MessageHelper.sendMessageToChannel(
                    game.getMainGameChannel(), "In Red Tape, no objective is revealed at this stage.");
    ...
    public static void poScoring(
        ...
        for (Player player2 : Helper.getInitativeOrder(game)) {
            ...
            if (game.getStoredValue(key3).contains(player2.getFaction() + "*")) {
                message =
                        "wishes to score a public objective but has people ahead of them in initiative order who need to resolve first."
        ...
        if (buttonID.contains("foresight")) {
            MessageHelper.sendMessageToChannel(
                    ...
                    player.getFactionEmojiOrColor()
                            ...
                            + FactionEmojis.Naalu + "**Foresight** ability.");
            player.setStrategicCC(player.getStrategicCC() - 1);
            skilled = true;
        }
    ...
    public static void getAgentSelection(ButtonInteractionEvent event, String buttonID, Game game, Player player) {
        ...
        MessageHelper.sendMessageToChannelWithButtons(
                event.getMessageChannel(),
                player.getRepresentationUnfogged() + " choose the target of your agent",
                buttons);
    }
    ...
    public static void acDiscardFromHand(
            ...
            Game game,
            Player player,
            MessageChannel actionsChannel) { // TODO: bake this into /ac discard
            ...
            try {
                ...
                if (player.hasUnexhaustedLeader("cymiaeagent")) {
                    ...
                    Button hacanButton = Buttons.gray(
                            "exhaustAgent_cymiaeagent_" + player.getFaction(),
                            "Use Cymiae Agent",
    ...
    public static void acPlayFromHand(
            ButtonInteractionEvent event, String buttonID, Game game, Player player) { // TODO: bake this into /ac play
    ...
    public static void deleteButtons(ButtonInteractionEvent event, String buttonID, Game game, Player player) {
            ...
            if ("Done Redistributing Command Tokens".equalsIgnoreCase(buttonLabel)) {
                ...
                if ("leadership".equalsIgnoreCase(buttonID)) {
                    String message = playerRep + ", your initial command token allocation was " + shortCCs
                            + ". Your final command tokens allocation is " + finalCCs + ".";
        ...
        if (("Done Exhausting Planets".equalsIgnoreCase(buttonLabel)
            ...
            if ("Done Producing Units".equalsIgnoreCase(buttonLabel)) {
                ...
                String message2 =
                        player.getRepresentationUnfogged() + " Click the names of the planets you wish to exhaust.";
    ...
    private static void respondAllPlayersReacted(ButtonInteractionEvent event, Game game) {
        ...
        switch (buttonID) {
            ...
            case "no_when", "no_when_persistent" -> ReactionService.handleAllPlayersReactingNoWhens(
                    event.getInteraction().getMessage(), game);
            case "no_after", "no_after_persistent" -> ReactionService.handleAllPlayersReactingNoAfters(
                    event.getInteraction().getMessage(), game);
            case "no_sabotage" -> ReactionService.handleAllPlayersReactingNoSabotage(
                    event.getInteraction().getMessage(), game);
            case Constants.PO_SCORING, Constants.PO_NO_SCORING -> {
                ...
                if (game.isRedTapeMode()) {
                    message2 =
                            "All players have indicated scoring. This game is Red Tape mode, which means no objective is revealed at this stage."
    ...
    public static void placeHolderOfConInSystem(
        ...
        String message = player.getRepresentation() + " Placed 1 " + StringUtils.capitalize(color)
                + " command token in the "
    ...
    public static void startYinSpinner(ButtonInteractionEvent event, Player player, Game game) {
        ...
        String message =
                "Use buttons to drop 2 infantry on a planet. Technically you may also drop 2 infantry with your ships, but this ain't supported yet via button."; // TODO: support yin spinner into space
        ButtonHelper.deleteTheOneButton(event);
    ...
    public static void useLawsOrder(ButtonInteractionEvent event, Player player, Game game) {
        MessageHelper.sendMessageToChannel(
                event.getChannel(),
                player.getFactionEmojiOrColor() + " is paying " + MiscEmojis.Influence_1
                        + " influence to ignore laws for the turn.");
        List<Button> buttons = ButtonHelper.getExhaustButtonsWithTG(game, player, "inf");
        Button doneExhausting = Buttons.red("deleteButtons_spitItOut", "Done Exhausting Planets");
        buttons.add(doneExhausting);
        MessageHelper.sendMessageToChannelWithButtons(
                event.getChannel(),
                "Click the names of the planets you wish to exhaust to pay the 1 influence",
                buttons);
        ButtonHelper.deleteTheOneButton(event);
    ...
    public static void dominusOrb(ButtonInteractionEvent event, Player player, Game game) {
        ...
        MessageHelper.sendMessageToChannel(
                event.getMessageChannel(), purgeOrExhaust + ExploreEmojis.Relic + " relic: " + relicName);
        ButtonHelper.deleteMessage(event);
    ...
    public static void chooseDifferentDestination(ButtonInteractionEvent event, Player player, Game game) {
        String message =
                "Choosing a different system to activate. Please select the ring of the map that the system you wish to activate is located in."
    ...
    public static void proceedToStrategy(ButtonInteractionEvent event, Game game) {
        ...
        if (game.getStoredValue("agendaChecksNBalancesAgainst").isEmpty()) {
            ...
            MessageHelper.sendMessageToChannel(
                    event.getChannel(), "All planets have been readied at the end of the agenda phase.");
        } else {
            MessageHelper.sendMessageToChannel(
                    event.getChannel(),
    ...
    public static void diploSystem(Player player, Game game) {
        String message =
                player.getRepresentationUnfogged() + " Click the name of the planet whose system you wish to diplo";
    ...
    public static void redistributeCCButtons(ButtonInteractionEvent event, Player player, Game game) {
        ...
        Button doneGainingCC =
                Buttons.red(finsFactionCheckerPrefix + "deleteButtons", "Done Redistributing Command Tokens");
        ...
        if ("statusHomework".equalsIgnoreCase(game.getPhaseOfGame())) {
            ...
            if (player.hasAbility("versatile") || player.hasTech("hm") || cyber) {
                ...
                if (properGain > 2) {
                    MessageHelper.sendMessageToChannel(
                            player.getCardsInfoThread(),
                            "Heads up " + player.getRepresentationUnfogged() + ", the bot thinks you should gain "
                                    + properGain + " command token" + (properGain == 1 ? "" : "s") + " now due to: "
                                    + reasons + ".");
                }
            }
    ...
    public static void applytempcombatmodAC(ButtonInteractionEvent event, Player player, String buttonID) {
        ...
        TemporaryCombatModifierModel combatModAC =
                CombatTempModHelper.getPossibleTempModifier(Constants.AC, acAlias, player.getNumberTurns());
        if (combatModAC != null) {
    ...
    public static void applytempcombatmodtech(ButtonInteractionEvent event, Player player) {
        String acAlias = "sc";
        TemporaryCombatModifierModel combatModAC =
                CombatTempModHelper.getPossibleTempModifier("tech", acAlias, player.getNumberTurns());
        if (combatModAC != null) {
            player.addNewTempCombatMod(combatModAC);
            MessageHelper.sendMessageToChannel(
                    event.getChannel(),
