...
class ExploreButtonHandler {
    ...
    public static void resolveVolatileFuelSource(
            String buttonID, Game game, Player player, ButtonInteractionEvent event) {
        ...
        if (!failed) {
            String message = player.getRepresentation() + ", " + mechOrInfCheckMessage
        ...
        if (!failed && !event.getMessage().getContentRaw().contains("fragment")) {
            ...
            if (!game.isFowMode() && event.getChannel() != game.getActionsChannel()) {
                MessageHelper.sendMessageToChannel(
                        player.getCorrectChannel(), player.getFactionEmoji() + " " + mechOrInfCheckMessage);
            }
        }
    }
    ...
    public static void resolveCoreMine(String buttonID, Game game, Player player, ButtonInteractionEvent event) {
        ...
        if (!failed) {
            message =
                    player.getRepresentation() + ", " + message + "Gained 1 trade good " + player.gainTG(1, true) + ".";
    ...
    public static void resolveWarForgeRuins(Game game, String buttonID, Player player, ButtonInteractionEvent event) {
        ...
        if ("mech".equalsIgnoreCase(mech)) {
            ...
            message = player.getRepresentation() + ", " + message + "Placed mech on"
                    + Mapper.getPlanet(planet).getName();
        } else {
            AddUnitService.addUnits(
                    event, game.getTileFromPlanet(planet), game, player.getColor(), "2 infantry " + planet);
            message = player.getRepresentation() + ", " + message + "Placed " + UnitEmojis.infantry
                    + UnitEmojis.infantry + " on" + Mapper.getPlanet(planet).getName();
        }
        ReactionService.addReaction(event, game, player, message);
        ButtonHelper.deleteMessage(event);
    }
    ...
    public static void resolveWarForgeRuinsMech(
        ...
        String message = player.getRepresentation() + " is using a mech to resolve _War Forge Ruins_." + " Placing "
                + placedUnit + " on " + Helper.getPlanetRepresentation(planetID, game) + ".";
        MessageHelper.sendMessageToChannel(event.getChannel(), message);
        ButtonHelper.deleteMessage(event);
    }
    ...
    public static void resolveSeedySpace(Game game, String buttonID, Player player, ButtonInteractionEvent event) {
        ...
        if ("ac".equalsIgnoreCase(acOrAgent)) {
            if (player.hasAbility("scheming")) {
                ...
                game.drawActionCard(player.getUserID());
                message = player.getRepresentation() + ", " + message
            ...
            message = player.getRepresentation() + ", " + message + "Readied "
                    + Mapper.getLeader(acOrAgent).getName();
        }
        ReactionService.addReaction(event, game, player, message);
        ButtonHelper.deleteMessage(event);
    }
