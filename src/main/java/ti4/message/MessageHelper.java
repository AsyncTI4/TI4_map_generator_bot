...
public class MessageHelper {
    ...
    private static void addFactionReactToMessage(Game game, Player player, Message message) {
        ...
        if (reactionEmoji != null) {
            message.addReaction(reactionEmoji)
                    .queue(
                            null,
                            error -> BotLogger.log(getRestActionFailureMessage(
    ...
    public static void sendMessageToChannelAndPin(MessageChannel channel, String messageText) {
        MessageFunction pin = (msg) -> msg.pin()
                .queue(
                        null,
                        error -> BotLogger.log(
                                getRestActionFailureMessage(channel, "Failed to pin message", null, error)));
            ...
            if (iterator.hasNext()) { // not last message
                channel.sendMessage(messageCreateData)
                        .queue(
                                null,
                                error -> BotLogger.log(getRestActionFailureMessage(
                                        channel, "Failed to send intermediate message", messageCreateData, error)));
                ...
                channel.sendMessage(messageCreateData)
                        .queue(
                                ...
                                error -> BotLogger.log(getRestActionFailureMessage(
                                        channel, finalMessageText, messageCreateData, error)));
            }
        }
    }
    ...
    private static List<List<MessageEmbed>> getPartitionedEmbedLists(List<MessageEmbed> embeds) {
        ...
        return ListUtils.partition(
                embeds, 8); // max is 10, but we've had issues with 6k char limit in embeds in single message
    }
