...
public class TileGenerator {
    ...
    public BufferedImage draw(Tile tile, TileStep step) {
        ...
        var isSpiral = tile.getTileModel().getShipPositionsType() != null
                && tile.getTileModel().getShipPositionsType().isSpiral();
        switch (step) {
            ...
            case Tile -> {
                ...
                ShipPositionModel.ShipPosition shipPositionsType =
                        TileHelper.getTileById(tile.getTileID()).getShipPositionsType();
                ...
                if (!game.getHexBorderStyle().equals("off") && controllingPlayer != null && !isSpiral) {
                    ...
                    BufferedImage border = DrawingUtil.hexBorder(
                            game.getHexBorderStyle(), Mapper.getColor(controllingPlayer.getColor()), openSides);
                ...
                if (reconstruction && (Mapper.getWormholes(tile.getTileID()).contains(Constants.BETA))) {
                    BufferedImage doubleWormholeImage = ImageHelper.readScaled(
                ...
                if ((ButtonHelper.isLawInPlay(game, "nexus") || ButtonHelper.isLawInPlay(game, "absol_nexus"))
                        ...
                        && !(ButtonHelper.isLawInPlay(game, "travel_ban")
                                || ButtonHelper.isLawInPlay(
                                        game,
                                        "absol_travelban"))) // avoid doubling up, which is important when using the
                // transparent symbol
                {
                    BufferedImage blockedWormholeImage = ImageHelper.read(ResourceHelper.getInstance()
                            .getTokenFile("agenda_wormhole_blocked" + (reconstruction ? "_half" : "") + ".png"));
            ...
            case Extras -> {
                ...
                game.getBorderAnomalies().forEach(borderAnomalyHolder -> {
                    if (borderAnomalyHolder.getTile().equals(tile.getPosition())) {
                        addBorderDecoration(
                                borderAnomalyHolder.getDirection(), null, tileGraphics, borderAnomalyHolder.getType());
                    }
                });
            }
            case Units -> {
                ...
                UnitHolder spaceUnitHolder = unitHolders.stream()
                        .filter(unitHolder -> unitHolder.getName().equals(Constants.SPACE))
                        .findFirst()
                        .orElse(null);
                ...
                if (prodInSystem > 0 && game.isShowGears() && !game.isFowMode()) {
                    ...
                    tileGraphics.drawImage(
                            gearImage,
                            TILE_PADDING + TILE_POSITION_POINT.x + xMod - 29,
                            TILE_PADDING + TILE_POSITION_POINT.y + yMod - 4,
                            null);
                    tileGraphics.setFont(Storage.getFont35());
                    tileGraphics.drawString(
                            prodInSystem + "",
                            TILE_PADDING + TILE_POSITION_POINT.x + xMod + 15 + textModifer - 25,
                            TILE_PADDING + TILE_POSITION_POINT.y + yMod + 40);
                }
            ...
            case Distance -> {
                ...
                if (distance < 11) {
                    ...
                    DrawingUtil.superDrawString(
                            tileGraphics,
                            distance.toString(),
                            x + tileImage.getWidth() / 2,
                            y + tileImage.getHeight() / 2,
                            Color.WHITE,
                            MapGenerator.HorizontalAlign.Center,
                            MapGenerator.VerticalAlign.Center,
                            stroke4,
            ...
            case Aetherstream -> {
                ...
                if (!anomalyIsAdjacent) {
                    ...
                    x += (isSpiral ? 36 : 0);
                    y += (isSpiral ? 43 : 0);
                    String batFile = ResourceHelper.getInstance()
                            .getGeneralFile("zobat" + (RandomHelper.isOneInX(4096) ? "_shiny" : "") + ".png");
            ...
            case Empties -> {
                ...
                if (!isEmpty) {
                    ...
                    DrawingUtil.drawCenteredString(
                            tileGraphics, "!", new Rectangle(x, y, 159, 159), Storage.getFont80());
                }
            }
            case SpaceCannon -> {
                if (game.isFowMode()) {
                ...
                for (Player player : game.getRealPlayers()) {
                    ...
                    if (player.hasAbility("starfall_gunnery")) {
                        for (int i = ButtonHelper.checkNumberNonFighterShipsWithoutSpaceCannon(player, tile);
                                i > 0;
                                i--) {
                            diceCount.add(8 - mod);
                        }
                    ...
                    for (String adjTilePos : FoWHelper.getAdjacentTiles(game, tilePos, player, false, true)) {
                        ...
                        for (UnitHolder unitHolder : adjTile.getUnitHolders().values()) {
                            if (sameTile && Constants.MECATOLS.contains(unitHolder.getName())) {
                                if (player.controlsMecatol(false)
                                        && player.getTechs().contains("iihq")) {
                                    diceCount.add(5 - mod);
                                }
                            }
                            for (Map.Entry<Units.UnitKey, Integer> unitEntry :
                                ...
                                if (game.getPlayerByColorID(unitKey.getColorID())
                                                .orElse(null)
                                        != player) {
                                    continue;
                                }
                ...
                if (pdsDice.isEmpty()) {
                    ...
                    for (Player player : pdsDice.keySet()) {
                        ...
                        BufferedImage bufferedImage =
                                ImageHelper.readScaled(Mapper.getCCPath(Mapper.getControlID(player.getColor())), scale);
                        DrawingUtil.drawControlToken(tileGraphics, bufferedImage, player, x, y, false, scale / 2);
                        DrawingUtil.drawCenteredString(
                                tileGraphics,
                                numberOfDice + (rerolls ? "*" : ""),
                                new Rectangle(
                                        Math.round(x + 6 * scale),
                                        Math.round(y + 12 * scale),
                                        Math.round(61 * scale / 2),
                                        Math.round(24 * scale * 2 / 3)),
                                bigFont);
                        DrawingUtil.drawCenteredString(
                                tileGraphics,
                                "(" + expectedHits + ")",
                                new Rectangle(
                                        Math.round(x + 6 * scale),
                                        Math.round(y + 12 * scale + 24 * scale * 2 / 3),
                                        Math.round(61 * scale / 2),
                                        Math.round(24 * scale / 3)),
                                smallFont);
                        if (numberOfDice >= 5) {
                            DrawingUtil.drawCenteredString(
                                    tileGraphics,
                                    pdsDice.get(player).subList(0, numberOfDice / 3).stream()
                                                    .map(Object::toString)
                                                    .collect(Collectors.joining(","))
                                            + ",",
                                    new Rectangle(
                                            Math.round(x + 73 * scale / 2),
                                            Math.round(y + 6 * scale),
                                            Math.round(73 * scale / 2),
                                            Math.round(36 * scale / 3)),
                                    smallFont);
                            DrawingUtil.drawCenteredString(
                                    tileGraphics,
                                    pdsDice.get(player).subList(numberOfDice / 3, 2 * numberOfDice / 3).stream()
                                                    .map(Object::toString)
                                                    .collect(Collectors.joining(","))
                                            + ",",
                                    new Rectangle(
                                            Math.round(x + 73 * scale / 2),
                                            Math.round(y + 6 * scale + 36 * scale / 3),
                                            Math.round(73 * scale / 2),
                                            Math.round(36 * scale / 3)),
                                    smallFont);
                            DrawingUtil.drawCenteredString(
                                    tileGraphics,
                                    pdsDice.get(player).subList(2 * numberOfDice / 3, numberOfDice).stream()
                                            .map(Object::toString)
                                            .collect(Collectors.joining(",")),
                                    new Rectangle(
                                            Math.round(x + 73 * scale / 2),
                                            Math.round(y + 6 * scale + 36 * scale * 2 / 3),
                                            Math.round(73 * scale / 2),
                                            Math.round(36 * scale / 3)),
                                    smallFont);
                            ...
                            DrawingUtil.drawCenteredString(
                                    tileGraphics,
                                    pdsDice.get(player).subList(0, numberOfDice / 2).stream()
                                                    .map(Object::toString)
                                                    .collect(Collectors.joining(","))
                                            + ",",
                                    new Rectangle(
                                            ...
                                            Math.round(y + 12 * scale),
                                            Math.round(73 * scale / 2),
                                            Math.round(24 * scale / 2)),
                                    smallFont);
                            DrawingUtil.drawCenteredString(
                                    tileGraphics,
                                    pdsDice.get(player).subList(numberOfDice / 2, numberOfDice).stream()
                                            .map(Object::toString)
                                            .collect(Collectors.joining(",")),
                                    new Rectangle(
                                            Math.round(x + 73 * scale / 2),
                                            Math.round(y + 12 * scale + 24 * scale / 2),
                                            Math.round(73 * scale / 2),
                                            Math.round(24 * scale / 2)),
                                    smallFont);
                        } else {
                ...
                for (Planet planet : tile.getPlanetUnitHolders()) {
                    ...
                    if (tile.isMecatol()) {
                        ...
                        traitFile = ResourceHelper.getInstance()
                                .getFactionFile(
                                        Mapper.getPlanet(planet.getName()).getFactionHomeworld() + ".png");
                    ...
                    if (!traitFile.isEmpty()) {
                        ...
                        for (int i = -padding; i < w + padding; i++) {
                            for (int j = -padding; j < h + padding; j++) {
                                ...
                                backgroundInner.setRGB(
                                        i + padding, j + padding, new Color(0, 0, 0, bestInnerAlpha).getRGB());
                                backgroundOuter.setRGB(
                                        i + padding, j + padding, new Color(255, 255, 255, bestOuterAlpha).getRGB());
                            }
                        }

                        Point position = planet.getHolderCenterPosition();
                        if (planet.getName().equalsIgnoreCase("mirage")
                                && (tile.getPlanetUnitHolders().size() == 3 + 1)) {
                            position = new Point(
                                    Constants.MIRAGE_TRIPLE_POSITION.x + Constants.MIRAGE_CENTER_POSITION.x,
                    ...
                    for (String skip : skips) {
                        String skipFile =
                                switch (skip.toLowerCase()) {
                                    case "biotic" -> ResourceHelper.getInstance()
                                            .getGeneralFile("Biotic light.png");
                                    case "cybernetic" -> ResourceHelper.getInstance()
                                            .getGeneralFile("Cybernetic light.png");
                                    case "propulsion" -> ResourceHelper.getInstance()
                                            .getGeneralFile("Propulsion_light.png");
                                    case "warfare" -> ResourceHelper.getInstance()
                                            .getGeneralFile("Warfare_light.png");
                        ...
                        bufferedImage = ImageHelper.scale(bufferedImage, (float) Math.sqrt(9200.0f
                                / bufferedImage.getWidth()
                                / bufferedImage.getHeight()
                                / Math.sqrt(1.0f * number)));
                        int w = bufferedImage.getWidth();
                        int h = bufferedImage.getHeight();
                        int innerBorder = 3;
                        ...
                        if (number > 1) {
                            position = new Point(
                                    position.x - 20 + count * 40 / (number - 1),
                                    position.y - 20 + count * 40 / (number - 1));
                        }
                        position = new Point(position.x - w / 2 + TILE_PADDING, position.y - h / 2 + TILE_PADDING);
            ...
            case TileNumber -> {
                ...
                DrawingUtil.superDrawString(
                        ...
                        textY,
                        Color.WHITE,
                        MapGenerator.HorizontalAlign.Right,
                        MapGenerator.VerticalAlign.Bottom,
                        stroke7,
                        Color.BLACK);
            }
    ...
    private void setTextSize(Graphics tileGraphics) {
        if (game.getTextSize().equals("large")) {
            tileGraphics.setFont(Storage.getFont40());
        } else if (game.getTextSize().equals("medium")) {
            tileGraphics.setFont(Storage.getFont30());
        } else if (game.getTextSize().equals("tiny")) {
            ...
            for (String controlID : controlList) {
                ...
                if (position != null) {
                    ...
                    if (player != null
                        ...
                        DrawingUtil.drawControlToken(
                                tileGraphics, exhaustedTokenImage, player, imgX, imgY, convertToGeneric, scale);
                        rectangles.add(
                                new Rectangle(imgX, imgY, controlTokenImage.getWidth(), controlTokenImage.getHeight()));
                    }
    ...
    private static void addSleeperToken(
            Tile tile, Graphics tileGraphics, UnitHolder unitHolder, Function<String, Boolean> isValid, Game game) {
        BufferedImage tokenImage;
        Point centerPosition = unitHolder.getHolderCenterPosition();
        if (unitHolder.getName().equalsIgnoreCase("mirage")
                && (tile.getPlanetUnitHolders().size() == 3 + 1)) {
        ...
        for (String tokenID : tokenList) {
            if (isValid.apply(tokenID)) {
                ...
                Point position = new Point(
                        centerPosition.x - (tokenImage.getWidth() / 2),
    ...
    private static void addPlanetToken(
            Tile tile, Graphics tileGraphics, UnitHolder unitHolder, List<Rectangle> rectangles) {
        List<String> tokenList = new ArrayList<>(unitHolder.getTokenList());
        tokenList.sort((o1, o2) -> {
            ...
            for (String tokenID : tokenList) {
                ...
                if (tokenPath.contains(Constants.WORLD_DESTROYED)
                    ...
                    tileGraphics.drawImage(
                            tokenImage,
                            TILE_PADDING + centerPosition.x - (tokenImage.getWidth() / 2),
                            TILE_PADDING + centerPosition.y - (tokenImage.getHeight() / 2) + 10,
                            null);
                } else if (tokenPath.contains(Constants.CUSTODIAN_TOKEN)) {
                    ...
                    if (position != null) {
                        ...
                        rectangles.add(new Rectangle(
                                TILE_PADDING + position.x,
                                TILE_PADDING + position.y,
                                tokenImage.getWidth(),
                                tokenImage.getHeight()));
                    } else {
                        tileGraphics.drawImage(
                                tokenImage,
                                TILE_PADDING + centerPosition.x + xDelta,
                                TILE_PADDING + centerPosition.y,
                                null);
                        rectangles.add(new Rectangle(
                                TILE_PADDING + centerPosition.x + xDelta,
                                TILE_PADDING + centerPosition.y,
    ...
    private static void oldFormatPlanetTokenAdd(
            Tile tile, Graphics tileGraphics, UnitHolder unitHolder, List<String> tokenList) {
        int deltaY = 0;
        int offSet = 0;
        Point centerPosition = unitHolder.getHolderCenterPosition();
        ...
        for (String tokenID : tokenList) {
            ...
            tileGraphics.drawImage(
                    image,
                    TILE_PADDING + x - (image.getWidth() / 2),
                    TILE_PADDING + y + offSet + deltaY - (image.getHeight() / 2),
                    null);
            y += image.getHeight();
        }
    }
        ...
        for (String tokenID : tokenList) {
            ...
            if (tokenPath.contains(Constants.MIRAGE)) {
                if (tile.getPlanetUnitHolders().size() == 3 + 1) {
                    tileGraphics.drawImage(
                            tokenImage,
                            TILE_PADDING + Constants.MIRAGE_TRIPLE_POSITION.x,
                            TILE_PADDING + Constants.MIRAGE_TRIPLE_POSITION.y,
                            null);
                } else {
                    tileGraphics.drawImage(
                            tokenImage,
                            TILE_PADDING + Constants.MIRAGE_POSITION.x,
                            TILE_PADDING + Constants.MIRAGE_POSITION.y,
                            null);
                }
                ...
                if ((ButtonHelper.isLawInPlay(game, "travel_ban") || ButtonHelper.isLawInPlay(game, "absol_travelban"))
                        && (tokenPath.toLowerCase().contains("alpha")
                                || tokenPath.toLowerCase().contains("beta"))) {
                    BufferedImage blockedWormholeImage = ImageHelper.read(ResourceHelper.getInstance()
                            .getTokenFile("agenda_wormhole_blocked" + (reconstruction ? "_half" : "") + ".png"));
    ...
    private static void drawOnWormhole(Tile tile, Graphics graphics, BufferedImage icon, int offset, String types) {
        switch (tile.getTileID()) {
            ...
            case "m32": // Vespa/Apis
                ...
                break;
            default:
                Point wormholeLocation = TileHelper.getTileById(tile.getTileID())
                        .getShipPositionsType()
                        .getWormholeLocation();
                if (wormholeLocation == null) {
                    ...
                    graphics.drawImage(
                            icon,
                            TILE_PADDING + offset + wormholeLocation.x,
                            TILE_PADDING + offset + wormholeLocation.y,
                            null);
                }
        }
    }
