...
public class MessageListener extends ListenerAdapter {
    ...
    public void onMessageReceived(@Nonnull MessageReceivedEvent event) {
        ...
        ExecutorServiceManager.runAsync("MessageListener task", EXECUTION_TIME_WARNING_THRESHOLD_SECONDS, () -> processMessage(event, message));
    }
    ...
    private static Player getPlayer(MessageReceivedEvent event, Game game) {
        ...
        for (Player player2 : game.getRealPlayers()) {
            ...
            if (player2.getTeamMateIDs().contains(event.getMember().getId())) {
                return player2;
            }
        }
        return player;
