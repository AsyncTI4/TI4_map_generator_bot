    ...
    private static <T extends ListenerContext> Consumer<T> buildConsumer(Method method, Function<T, List<Object>> getArgs, boolean save) {
        return context -> {
            ...
            try {
                ...
                BotLogger.error("Error within handler \"" + method.getDeclaringClass().getSimpleName() + "#" + method.getName() + "\":", e.getCause(), true);
                for (Object arg : args) {
    ...
    public static <C extends ListenerContext, H extends Annotation> Map<String, Consumer<C>> findKnownHandlers(Class<C> contextClass, Class<H> handlerClass) {
        ...
        try {
            if (!handlers().contains(handlerClass)) {
                BotLogger.warning("Unknown handler class `" + handlerClass.getName() + "`. Please fix " + Constants.jazzPing(), false);
                return consumers;
            }
            if (!contexts().contains(contextClass)) {
                BotLogger.warning("Unknown context class `" + contextClass.getName() + "`. Please fix " + Constants.jazzPing(), false);
                return consumers;
            }
            for (Class<?> klass : AsyncTI4DiscordBot.getAllClasses()) {
                for (Method method : klass.getDeclaredMethods()) {
                    ...
                    if (!Modifier.isStatic(method.getModifiers())) {
                        BotLogger.warning("Method `" + methodName + "` is not static. Please fix it " + Constants.jazzPing(), false);
                        continue;
                    }
            ...
            BotLogger.error(Constants.jazzPing() + " bot cannot read methods in the file.", e, true);
        } catch (Exception e) {
            BotLogger.error(Constants.jazzPing() + " some other issue registering buttons.", e, true);
        }

        BotLogger.info("Registered " + consumers.size() + " handlers of type " + handlerClass.getName(), true);
        return consumers;
    }
