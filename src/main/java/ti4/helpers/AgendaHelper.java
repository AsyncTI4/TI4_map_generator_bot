        ...
        if (activeMap.getCurrentAgendaInfo().startsWith("Law")) {
                ...
                if (agID.equalsIgnoreCase("regulations")) {
                    if (winner.equalsIgnoreCase("for")) {
                        ...
                        MessageHelper.sendMessageToChannel(activeMap.getMainGameChannel(),
                            Helper.getGamePing(activeMap.getGuild(), activeMap) + " Reduced people's fleets to 4 if they had more than that");
                    } else {
                        for (Player playerB : activeMap.getRealPlayers()) {
            ...
            if (activeMap.getCurrentAgendaInfo().contains("Player")) {
                ...
                if (agID.equalsIgnoreCase("grant_reallocation")) {
                    ...
                    buttons.add(getTech);
                    MessageHelper.sendMessageToChannelWithButtons(ButtonHelper.getCorrectChannel(player2, activeMap),
                        Helper.getPlayerRepresentation(player2, activeMap) + " Use the button to get a tech", buttons);
                }
                    ...
                    if (winningR != null && (specificVote.contains("Rider") || winningR.hasAbility("future_sight"))) {
                        ...
                        if (specificVote.contains("Galactic Threat Rider")) {
                            ...
                            MessageHelper.sendMessageToChannelWithButtons(channel, identity + " resolve Galactic Threat Rider using the buttons",
                                ButtonHelperFactionSpecific.getButtonsForPossibleTechForNekro(winningR, potentialTech, activeMap));
                        }
    ...
    public static List<Button> getPlanetRefreshButtons(GenericInteractionCreateEvent event, Player player, Map activeMap) {
        ...
        for (String planet : planets) {
            Button button = Button.success("refresh_" + planet,
                StringUtils.capitalize(planet) + "(" + Helper.getPlanetResources(planet, activeMap) + "/" + Helper.getPlanetInfluence(planet, activeMap) + ")");
            planetButtons.add(button);
        }
