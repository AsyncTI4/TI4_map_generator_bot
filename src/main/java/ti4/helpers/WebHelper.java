...
public class WebHelper {
    ...
    static {
        ...
        try (InputStream input = new FileInputStream(Objects.requireNonNull(ResourceHelper.getInstance().getWebFile("web.properties")))) {
            ...
            BotLogger.error("Could not load web properties.", e, true);
        }
    }
    ...
    public static void putData(String gameName, Game game) {
        ...
        try {
            ...
            httpClient.sendAsync(request, HttpResponse.BodyHandlers.ofString())
                .exceptionally(e -> {
                    BotLogger.error("An exception occurred while performing an async send of game data to the website.", e, true);
                    return null;
                });
        } catch (IOException e) {
            BotLogger.error("Could not put data to web server", e, true);
        }
    }
    ...
    public static void putOverlays(String gameId, List<WebsiteOverlay> overlays) {
        ...
        try {
            ...
            s3AsyncClient.putObject(request, AsyncRequestBody.fromString(json))
                .exceptionally(e -> {
                    BotLogger.error("An exception occurred while performing an async send of overlay data to the website.", e, true);
                    return null;
                });
        } catch (Exception e) {
            BotLogger.error("Could not put overlay to web server", e, true);
        }
    }
    ...
    public static void putStats() {
        ...
        for (ManagedGame managedGame : GameManager.getManagedGames()) {
            if (managedGame.getRound() > 2 || managedGame.isHasEnded() && managedGame.isHasWinner()) {
                ...
                try {
                    ...
                    BotLogger.error("Failed to create GameStatsDashboardPayload for game: `" + managedGame.getName() + "`", e, true);
                }
            }
        }
        ...
        try {
            ...
            s3AsyncClient.putObject(request, AsyncRequestBody.fromString(json))
                .exceptionally(e -> {
                    BotLogger.error("An exception occurred while performing an async send of game stats to the website.", e, true);
                    return null;
                });
        } catch (Exception e) {
            BotLogger.error("Could not put statistics to web server", e, true);
        }
    }
    ...
    public static void putMap(String gameName, byte[] imageBytes, boolean frog, Player player) {
        ...
        try {
            ...
            s3AsyncClient.putObject(request, AsyncRequestBody.fromBytes(imageBytes))
                .exceptionally(e -> {
                    BotLogger.error("An exception occurred while performing an async send of the game image to the website.", e, true);
                    return null;
                });
        } catch (SdkClientException e) {
            BotLogger.error("Could not add image for game `" + gameName + "` to web server. Likely invalid credentials.", e, true);
        } catch (Exception e) {
            BotLogger.error("Could not add image for game `" + gameName + "` to web server", e, true);
        }
    }
}
