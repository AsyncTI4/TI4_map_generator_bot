...
public abstract class SettingsMenu {
    ...
    protected void buttonFailed(GenericInteractionCreateEvent event, String userMsg, boolean pingJazz) {
        if (pingJazz) {
            BotLogger.error(event, userMsg + "\n" + Constants.jazzPing() + " Menu Framework button has failed.", false);
    ...
    private List<Button> settingsButtonsPage(int allottedSpace, int pageNum) {
        ...
        if (allottedSpace < allButtons.size()) {
            if (allottedSpace < 3) {
                ...
                BotLogger.error("NOT ENOUGH SPACE FOR BUTTONS IN MENU: " + navId(), true);
                return Collections.emptyList();
            }
    ...
    public String json() {
        ...
        try {
            ...
            BotLogger.error("Error mapping to json:", e, true);
        }
        return null;
    }
}
