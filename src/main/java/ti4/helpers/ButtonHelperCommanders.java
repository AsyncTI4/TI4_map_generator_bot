            ...
            for (Tile tile : game.getTileMap().values()) {
                for (UnitHolder uH : tile.getUnitHolders().values()) {
                    if (uH.getUnitDamageCount(UnitType.Mech, player.getColor()) > 0) {
                        ...
                        MessageHelper.sendMessageToChannel(ButtonHelper.getCorrectChannel(player, game), player.getRepresentation() + " repaired damaged mech in " + tile.getRepresentation() + " due to spending a strategy token");
                    }
                }
            }
        }
