...
public class ButtonHelperTacticalAction {
    ...
    public static void movingUnitsInTacticalAction(
        ...
        if (rest.contains("reverseall") || rest.contains("moveall")) {
            if (rest.contains("reverse")) {
                for (String unit : currentSystem.keySet()) {
                    ...
                    if (currentActivation.containsKey(unitkey)) {
                        game.setSpecificCurrentMovedUnitsFrom1TacticalAction(
                                unitkey, currentActivation.get(unitkey) - amount);
                    }
                            ...
                            if (unitKey.getUnitType() == UnitType.Infantry || unitKey.getUnitType() == UnitType.Mech) {
                                ...
                                if (damagedUnits > 0) {
                                    ...
                                    if (currentSystem.containsKey(rest)) {
                                        game.setSpecificCurrentMovedUnitsFrom1System(
                                                rest, currentSystem.get(rest) + amount);
                                    } else {
                                        game.setSpecificCurrentMovedUnitsFrom1System(rest, amount);
                                    }
                                    if (currentActivation.containsKey(unitName)) {
                                        game.setSpecificCurrentMovedUnitsFrom1TacticalAction(
                                                unitName, currentActivation.get(unitName) + amount);
                                    } else {
                                ...
                                if (amount > 0) {
                                    rest = unitName.toLowerCase() + "_"
                                            + unitHolder.getName().toLowerCase();
                                    if (currentSystem.containsKey(rest)) {
                                        game.setSpecificCurrentMovedUnitsFrom1System(
                                                rest, currentSystem.get(rest) + amount);
                                    } else {
                        ...
                        for (Map.Entry<UnitKey, Integer> unitEntry : units.entrySet()) {
                            ...
                            if (damagedUnits > 0) {
                                ...
                                if (currentActivation.containsKey(rest)) {
                                    game.setSpecificCurrentMovedUnitsFrom1TacticalAction(
                                            rest, currentActivation.get(rest) + amount);
                                } else {
            ...
            List<Button> systemButtons =
                    getButtonsForAllUnitsInSystem(player, game, game.getTileByPosition(pos), remove);
        ...
        if (currentActivation.containsKey(unitName)) {
            game.setSpecificCurrentMovedUnitsFrom1TacticalAction(unitName, currentActivation.get(unitName) + amount);
        } else {
            game.setSpecificCurrentMovedUnitsFrom1TacticalAction(unitName, amount);
        }
    ...
    public static void finishMovingForTacticalAction(
            Player player, Game game, ButtonInteractionEvent event, String buttonID) {
        ...
        if (game.getMovedUnitsFromCurrentActivation().isEmpty()
                ...
                for (Player nonActivePlayer : game.getRealPlayers()) {
                    ...
                    if (nonActivePlayer.getTechs().contains("vw")
                        ...
                        if (game.isFowMode()) {
                            ...
                            MessageHelper.sendMessageToChannel(
                                    player.getCorrectChannel(),
                                    player.getRepresentation() + ", you owe a promissory note to "
                                            + nonActivePlayer.getRepresentation() + " from triggering _Voidwatch_.");
                        }
                    }
                }
            }
            ...
            if (!game.getMovedUnitsFromCurrentActivation().isEmpty()
                ...
                Button empyButton =
                        Buttons.gray("exhaustAgent_empyreanagent", "Use Empyrean Agent", FactionEmojis.Empyrean);
    ...
    public static void finishMovingFromOneTile(
        ...
        MessageHelper.sendMessageToChannel(
                event.getMessageChannel(),
                "From system "
                        + tile.getRepresentationForButtons(game, player) + " (" + distance + " tile"
                        + (distance == 1 ? "" : "s") + " away)\n"
    ...
    public static void selectTileToMoveFrom(Player player, Game game, ButtonInteractionEvent event, String buttonID) {
        ...
        MessageHelper.sendMessageToChannelWithButtons(
                ...
                "Chose to move from "
                        ...
                        + ". Use buttons to select the units you wish to move.",
                systemButtons);
        ButtonHelper.deleteMessage(event);
    }
    ...
    public static void selectRingThatActiveSystemIsIn(Player player, Game game, ButtonInteractionEvent event) {
        ...
        if (!game.isFowMode() && game.getRingCount() < 5 && prefersDistanceBasedTacticalActions) {
            ...
            String message =
                    "Doing a tactical action. Please select the ring of the map that the system you wish to activate is located in."
    ...
    public static void alternateWayOfOfferingTiles(Player player, Game game) {
        ...
        List<Button> buttons = new ArrayList<>();
        String message =
                "Doing a tactical action. Please select the tile you wish to activate. Right now showing tiles ";
    ...
    public static void selectActiveSystem(Player player, Game game, ButtonInteractionEvent event, String buttonID) {
        ...
        if (!game.isFowMode()) {
            ...
            for (Player player_ : playersAdj) {
                String playerMessage = player_.getRepresentationUnfogged() + " - System "
                        + activeSystem.getRepresentationForButtons(game, player_) + " has been activated ";
        ...
        if (player.hasUnexhaustedLeader("l1z1xagent") && !button3.isEmpty() && !game.isL1Hero()) {
            String msg = player.getRepresentationUnfogged() + ", you can use buttons to resolve "
                    + (player.hasUnexhaustedLeader("yssarilagent") ? "Clever Clever " : "") + "I48S, the L1Z1Z"
                    + (player.hasUnexhaustedLeader("yssarilagent") ? "/Yssaril" : "") + " agent, if you so wish.";
            MessageHelper.sendMessageToChannelWithButtons(event.getMessageChannel(), msg, button3);
        }

        if (tile.isAnomaly()
                && player.getCommodities() < player.getCommoditiesTotal()
                && player.getActionCards().containsKey("harness")) {
            MessageHelper.sendMessageToChannel(
        ...
        MessageHelper.sendMessageToChannelWithButtons(
                event.getMessageChannel(),
                "Use buttons to select the first system you wish to move from.",
                systemButtons);
        ...
        if (player.hasAbility("recycled_materials")) {
            ...
            if (!buttons.isEmpty()) {
                MessageHelper.sendMessageToChannelWithButtons(
                        event.getMessageChannel(), "Use buttons to select which unit to recycle", buttons);
            }
        }
        if (player.hasRelic("absol_plenaryorbital")
            ...
            if (!buttons4.isEmpty()) {
                MessageHelper.sendMessageToChannelWithButtons(
                        event.getMessageChannel(), "You can place down the _Plenary Orbital_.", buttons4);
            }
        }
        if (!game.isFowMode()) {
            if (!game.isL1Hero()) {
    ...
    public static List<Button> getButtonsForAllUnitsInSystem(Player player, Game game, Tile tile, String moveOrRemove) {
        ...
        if ("Remove".equalsIgnoreCase(moveOrRemove)) {
            ...
            doAll = Buttons.gray(
                    finChecker + "unitTactical" + moveOrRemove + "_" + tile.getPosition() + "_moveAll",
                    "Move all units");
            concludeMove = Buttons.blue(
        ...
        for (String unit : displacedUnits.keySet()) {
            ...
            for (int x = 1; x < displacedUnits.get(origUnit) + 1; x++) {
                ...
                UnitType unitType = Units.findUnitType(
                        AliasHandler.resolveUnit(unitkey.toLowerCase().replace(" ", "")));
