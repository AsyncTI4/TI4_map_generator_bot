...
public class ButtonHelperTacticalAction {
    ...
    public static void concludeTacticalAction(Player player, Game game, ButtonInteractionEvent event) {
        if (!game.isL1Hero() && !FOWPlusService.isVoid(game, game.getActiveSystem())) {
            ...
            if (player.hasAbility("cunning")) {
                ...
                String msg = player.getRepresentationUnfogged() + " you can use the buttons to place a trap on a planet";
    ...
    public static void buildWithTacticalAction(
        ...
        List<Button> buttons = Helper.getPlaceUnitButtons(event, player, game, game.getTileByPosition(pos), "tacticalAction", "place");
        ...
        if (Helper.getProductionValue(player, game, game.getTileByPosition(pos), false) > 0
            ...
            message3 += "Reminder that you have _Prophecy of Ixth_ and should produce at least 2 fighters if you wish to keep it. Its removal is not automated.\n";
        }
        ...
        for (UnitHolder uh : tile.getUnitHolders().values()) {
            ...
            for (UnitKey unitKey : unitsMovedFromUnitHolder.keySet()) {
                ...
                for (UnitState state : UnitState.values()) {
                    int amt = states.get(state.ordinal());
                    for (int x = 1; x <= Math.min(2, amt); x++) {
                        Button reverse = ButtonHelper.buildMoveUnitButton(player, tile, uh, state, unitKey, x, true, false);
                        buttons.add(reverse);
                    }
                }
            }
