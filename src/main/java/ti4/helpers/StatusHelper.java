...
public final class StatusHelper {
        ...
        for (Player player : game.getActionPhaseTurnOrder()) {
            ...
            if (!game.getStoredValue(keyV).isEmpty()
                ...
                if (scorables.isEmpty()) {
                    messageText =
                            player.toString() + ", the bot does not believe that you can score any public objectives.";
                } else {
                    if (Helper.canPlayerScorePOs(game, player)) {
                        messageText = player.toString()
            ...
            if (!game.getStoredValue(keyV).isEmpty() && sos.contains(Integer.parseInt(game.getStoredValue(keyV)))) {
                ...
                if (count > 0 && player.isRealPlayer() && !player.isNpc()) {
                    ...
                    String message =
                            player.toString() + " has opted not to score a secret objective at this point in time.";
    ...
    public static List<Player> getPlayersInScoringOrder(Game game) {
        if (game.hasFullPriorityTrackMode()) {
            return PriorityTrackHelper.getPriorityTrack(game).stream()
                    ...
                    .toList();
        } else if (game.containsPlanet(EronousPlanetService.CANTRIS_ID)) {
    ...
    public static void handleStatusPhaseMiddle(Game game) {
        ...
        for (Player p2 : game.getRealPlayers()) {
            ...
            String endOfRoundMessage = p2.toString();
            endOfRoundMessage +=
            ...
            if (obsidian != null
                ...
                String seetheMsg = obsidian.toString() + ", please destroy one infantry belonging to " + p2.toString()
                        + ", using _Seethe_.";
    ...
    private static void sendRemoveBreachButtons(Game game) {
        Predicate<Tile> hasBreach = t -> t.getSpaceUnitHolder().containsToken(Constants.TOKEN_BREACH_ACTIVE);
        ...
        for (Player p : game.getRealPlayers()) {
            ...
            if (!buttons.isEmpty()) {
                ...
                String msg = p.toString() + ", you may remove active Breaches from systems that contain your ships.";
    ...
    private static void sendNeuralParasiteButtons(Game game) {
        for (Player player : game.getRealPlayers()) {
            ...
            if (hasFrankenFirmParasite || hasBaseFirmParasite) {
                ...
                if (home != null) {
                    List<Button> buttons = new ArrayList<>();
                    String infantryAmount = hasFrankenFirmParasite ? "4gf" : "gf";
                    for (Planet planet : home.getPlanetUnitHolders()) {
                        String id = player.factionButtonChecker() + "placeOneNDone_skipbuild_" + infantryAmount + "_"
                                + planet.getName();
            ...
            if (game.isFrankenGame()
                ...
                if (!buttons.isEmpty()) {
                    TechnologyModel parasiteModel = Mapper.getTech("parasite-obs_y");
                    String parasiteMsg = player.getRepresentationUnfogged() + ", a reminder to do "
                            + parasiteModel.getNameRepresentation() + ".";
                    MessageHelper.sendMessageToChannelWithButtons(player.getCorrectChannel(), parasiteMsg, buttons);
                }
        ...
        for (Player player : game.getRealPlayers()) {
            if (player.getStasisInfantry() > 0 && player.hasUnit("tf-yinclone")) {
                if (!ButtonHelper.getPlaceStatusInfButtons(game, player).isEmpty()) {
                    ...
                    String msg = player.toString() + ", please use these buttons to revive infantry. You have "
    ...
    private static void handleMonumentToTheAges(Game game) {
        ...
        for (Tile tile : game.getTiles()) {
            for (Planet planet : tile.getPlanetUnitHolders()) {
                if (planet.hasUnit(UnitType.Spacedock, neutral)) {
                    ...
                    if (remaining % 3 == 0) {
                        ...
                        Player p1 = neutral;
                        for (Player p : game.getRealPlayers()) {
                            if (p.containsPlanet(planet.getName())) {
                                p1 = p;
                                break;
                            }
                        }
        ...
        for (Player player : game.getRealPlayers()) {
            ...
            for (Tile tile : game.getTiles()) {
                for (UnitHolder unitHolder : tile.getUnitHolderValues()) {
                    ...
                    if (unitHolder.hasUnit(UnitType.Flagship, colorID)) {
    ...
    public static void poScoring(
        ...
        String key3b = "potentialScoreSOBlockers";
        String message;
        for (Player player2 : getPlayersInScoringOrder(game)) {
            if (player2 == player) {
                if (game.getStoredValue(key2).contains(player.getFaction() + "*")) {
                    game.setStoredValue(key2, game.getStoredValue(key2).replace(player.getFaction() + "*", ""));
                }
        ...
        if (channel != null) {
            ...
            if (player.getSecret(soIndex2) != null
                ...
                for (Player player2 : getPlayersInScoringOrder(game)) {
                    ...
                    if (game.getStoredValue(key3).contains(player2.getFaction() + "*")
                            || game.getStoredValue(key3b).contains(player2.getFaction() + "*")) {
                        message = player.toString() + " has been queued to score a secret objective. ";
                        if (!game.isFowMode()) {
