...
public class ButtonHelper {
    ...
    public static int resolveOnActivationEnemyAbilities(
        ...
        if (player.hasAbility("void_tap")
            && (activeSystem.getPlanetUnitHolders().isEmpty()
                || ButtonHelper.doesPlayerHaveFSHere("eidolon_flagship", player, activeSystem))) {
        ...
        for (Player nonActivePlayer : game.getPlayers().values()) {
            ...
            if (nonActivePlayer.hasTechReady("nf")
                ...
                if (justChecking) {
                    ...
                    Button decline = Buttons.red(fincheckerForNonActive + "deleteButtons", "Decline To Use Nullification Field");
                    List<Button> buttons = List.of(gainTG, decline);
                    MessageHelper.sendMessageToChannelWithButtons(
                        channel,
                        ident
        ...
        if (issuePing) {
            if (!game.getStoredValue("violatedSystems").contains(tile.getPosition())) {
                if (capacityViolated || fleetSupplyViolated || structuresViolated) {
                    ...
                    FileUpload systemWithContext = new TileGenerator(game, null, null, 0, tile.getPosition(), player).createFileUpload();
        ...
        for (String pos2 : directlyAdjacentTiles) {
            ...
            if (tile == null) {
                for (String pos3 : PositionMapper.getAdjacentTilePositions(pos2)) {
                    ...
                    if (tile2 != null && tile2.isEdgeOfBoard(game) && !pos3.equalsIgnoreCase(pos) && !usedPos.contains(pos3)) {
                        if (game.isAgeOfExplorationMode() && tile2.isHomeSystem(game)) {
                            continue;
                        }
                        usedPos.add(pos3);
                        buttons.add(Buttons.green(
                            player.getFinsFactionCheckerPrefix() + "starChartsStep3_" + newTileID + "_" + tile2.getPosition()
                                + "_" + pos,
    ...
    public static List<Button> getPossibleRings(Player player, Game game) {
        ...
        if (centerTile != null && FOWPlusService.canActivatePosition("000", player, game)) {
            Button rex = Buttons.green(finChecker + "ringTile_000", centerTile.getRepresentationForButtons(game, player));
    ...
    public static void resolveFullFrontierExplore(
        ...
        if (player.hasAbility("voidsailors") || player.hasAbility("dark_weaver")) {
            ...
            Button resolveExplore2 = Buttons.green("lanefirAgentRes_Accept_frontier_" + tile.getPosition(), "Use Lanefir Agent");
            List<Button> buttons = List.of(resolveExplore1, resolveExplore2);
            String message = player.getRepresentationUnfogged()
    ...
    public static Button buildRepairUnitButton(
    ...
    public static List<Button> getButtonsForRemovingAllUnitsInSystem(
        ...
        boolean spaceCombatish = type.equalsIgnoreCase("courageouscombat")
            || type.equalsIgnoreCase("spacecombat")
            || type.equalsIgnoreCase("assaultcannoncombat");
        boolean combat = type.contains("combat");
        boolean oneButtonPerUnit = limitOne
            || spaceCombatish
            || type.equalsIgnoreCase("combat"); // space combat or generic unspecified combat
        for (UnitHolder unitHolder : tile.getUnitHolders().values()) {
            if (unitHolder instanceof Planet
                && (spaceCombatish
            ...
            for (UnitKey unitKey : unitHolder.getUnitKeys()) {
                ...
                if (type.equalsIgnoreCase("courageouscombat")
                ...
                boolean canDamage = !type.equalsIgnoreCase("courageouscombat")
                    && !type.equalsIgnoreCase("assaultcannoncombat")
    ...
    public static void resolveRollForThalnos(Player player, Game game, String buttonID, ButtonInteractionEvent event) {
        ...
        CombatRollType rollType = CombatRollType.combatround;
        Map<UnitModel, Integer> playerUnitsByQuantity = CombatRollService.getUnitsInCombat(tile, combatOnHolder, player, event, rollType, game);
        ...
        List<NamedCombatModifierModel> tempMods = new ArrayList<>(CombatTempModHelper.BuildCurrentRoundTempNamedModifiers(
