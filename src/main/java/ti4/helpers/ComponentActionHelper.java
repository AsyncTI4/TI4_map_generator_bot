...
public class ComponentActionHelper {
    ...
    public static List<Button> getAllPossibleCompButtons(Game game, Player p1, GenericInteractionCreateEvent event) {
            ...
            if (!leader.isExhausted() && !leader.isLocked() && !leader.isActive()) {
                ...
                if ("ACTION:".equalsIgnoreCase(leaderAbilityWindow) || leaderName.contains("Ssruu")) {
                    if (leaderName.contains("Ssruu")) {
                        ...
                        Button lButton = Buttons.gray(
                                finChecker + prefix + "leader_" + leaderID, "Use " + leaderName, factionEmoji);
                        compButtons.add(lButton);
                    }
        ...
        if ((p1.getUnitsOwned().contains("sigma_muaat_flagship_1")
                        || p1.getUnitsOwned().contains("sigma_muaat_flagship_1"))
                && p1.getStrategicCC() > 0
                && !ButtonHelper.getTilesOfPlayersSpecificUnits(game, p1, UnitType.Flagship)
                        .isEmpty()) {
            Button abilityButton = Buttons.green(
    ...
    public static void resolvePressedCompButton(Game game, Player p1, ButtonInteractionEvent event, String buttonID) {
        ...
        switch (firstPart) {
            ...
            case "leader" -> {
                ...
                if (buttonID.contains("agent")) {
                    ...
                    if (leadersThatNeedSpecialSelection.contains(buttonID)) {
                        ...
                        String message =
                                p1.getRepresentationUnfogged() + " Use buttons to select the user of the agent";
            ...
            case "ability" -> {
                if ("starForge".equalsIgnoreCase(buttonID)) {
                    ...
                    ButtonHelperCommanders.resolveMuaatCommanderCheck(
                            p1, game, event, FactionEmojis.Sol + "Orbital Drop");
                    ...
                    p1.setStrategicCC(p1.getStrategicCC() - 1);
                    ButtonHelperCommanders.resolveMuaatCommanderCheck(
                            p1, game, event, FactionEmojis.Muaat + " " + UnitEmojis.flagship + "The Inferno");
                    ...
                    if (p1.getStrategicCC() > 0) {
                        ...
                        p1.setStrategicCC(p1.getStrategicCC() - 1);
                        ButtonHelperCommanders.resolveMuaatCommanderCheck(
                                p1, game, event, FactionEmojis.kolume + "Meditation");
                        MessageHelper.sendMessageToChannel(event.getMessageChannel(), successMessage);
                    } else {
            ...
            case "generic" -> MessageHelper.sendMessageToChannel(
                    event.getMessageChannel(), "Doing unspecified component action.");
            case "absolMOW" -> {
                MessageHelper.sendMessageToChannel(
    ...
    private static void resolveRelicComponentAction(
        ...
        if (juniorRelics.contains(relicID)) { // EXHAUST THE RELIC
            ...
            MessageHelper.sendMessageToChannelWithButtons(
                    event.getMessageChannel(), "Use buttons to decide who to use JR-XS455-O on.", buttons2);
        ...
        switch (relicID) {
            ...
            case "nanoforge", "absol_nanoforge", "baldrick_nanoforge" -> ButtonHelperRelics.offerNanoforgeButtons(
                    player, game, event);
            ...
            case "passturn" -> MessageHelper.sendMessageToChannelWithButton(
                    event.getChannel(), null, Buttons.REDISTRIBUTE_CCs);
