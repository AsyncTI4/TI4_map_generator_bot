...
public class ButtonHelperFactionSpecific {
    ...
    public static void gheminaMechStart(Player player, Game game, String buttonID, ButtonInteractionEvent event) {
        ...
        List<Button> buttons = ButtonHelper.getPlanetExplorationButtons(
                game, (Planet) ButtonHelper.getUnitHolderFromPlanetName(planet, game), player);
        MessageHelper.sendMessageToChannel(
                player.getCorrectChannel(),
    ...
    public static void resolveEdynAgendaStuffStep1(Player player, Game game, List<Tile> tiles) {
        ...
        MessageHelper.sendMessageToChannelWithButtons(
                ...
                player.getRepresentationUnfogged()
                        + ", please tell the bot which Sigil system you wish to place another player's command token in.",
                buttons);
    }
    ...
    public static void resolveEdynAgendaStuffStep2(
        ...
        MessageHelper.sendMessageToChannelWithButtons(
                player.getCorrectChannel(),
                player.getRepresentationUnfogged()
                        + ", please tell the bot which player's command token you wish to place down.",
                buttons);
    }
                ...
                for (Tile tile : game.getTileMap().values()) {
                    ...
                    if (unitHolder.getUnitCount(UnitType.Cavalry, player.getColor()) > 0) {
                        if (unitHolder.getUnitDamage() != null
                                && unitHolder.getUnitDamage().get(Mapper.getUnitKey("cavalry", player.getColorID()))
                                        != null
                                && unitHolder.getUnitDamage().get(Mapper.getUnitKey("cavalry", player.getColorID()))
                                        > 0) {
    ...
    public static void nekroStealTech(Game game, Player player, ButtonInteractionEvent event, String buttonID) {
        ...
        if (p2.getPromissoryNotesInPlayArea().contains("antivirus")) {
            ...
            MessageHelper.sendMessageToChannelWithButtons(
                    player.getCorrectChannel(),
                    trueIdentity + " copy a technology from your opponent using the buttons.",
                    buttons);
        } else {
            MessageHelper.sendMessageToChannel(
                    player.getCorrectChannel(),
        ...
        if (game.isFowMode()) {
            ...
            MessageHelper.sendMessageToChannel(
                    ...
                    player.getRepresentationUnfogged() + " has _Stymie_'d " + p2.getRepresentationUnfogged()
                            + " in the tile: " + tile.getRepresentationForButtons(game, player) + ".");
        }
    }
    ...
    public static void resolveASNStep2(Game game, Player player, String buttonID, ButtonInteractionEvent event) {
        ...
        String msg =
                player.getFactionEmoji() + " is resolving _Agency Supply Network_ in tile " + tile.getRepresentation();
    ...
    public static void offerKeleresStartingTech(Player player, Game game, ButtonInteractionEvent event) {
        ...
        MessageHelper.sendMessageToChannelWithButtons(
                player.getCorrectChannel(),
                player.getRepresentationUnfogged()
                        + " use the buttons to gain another technology the other players have:",
                techs2);
    }
    ...
    public static void offerArgentStartingTech(Player player) {
        ...
        MessageHelper.sendMessageToChannelWithButtons(
                player.getCorrectChannel(),
                player.getRepresentationUnfogged()
                        + " use the buttons to choose one of the 3 starting Argent technologies.",
                techs);
        MessageHelper.sendMessageToChannelWithButtons(
                player.getCorrectChannel(),
                player.getRepresentationUnfogged()
                        + " use the buttons to choose another of the 3 starting Argent technologies.",
                techs2);
    }
    ...
    public static void offerWinnuStartingTech(Player player) {
        ...
        MessageHelper.sendMessageToChannelWithButtons(
                player.getCorrectChannel(),
                player.getRepresentationUnfogged() + " use the buttons to get a technology.",
                techs);
    }
    ...
    public static void resolveHacanMechTradeStepOne(
        ...
        MessageHelper.sendMessageToChannelWithButtons(
                ...
                hacan.getRepresentationUnfogged() + "Choose which planet to relocate your units to",
                buttons);
        event.getMessage().delete().queue();
    }
    ...
    public static void resolveProductionBiomesStep1(Player hacan, Game game, GenericInteractionCreateEvent event) {
        ...
        MessageHelper.sendMessageToChannelWithButtons(
                hacan.getCorrectChannel(),
                hacan.getRepresentationUnfogged() + " choose who should get the two trade goods.",
                buttons);
    }
    ...
    public static void resolveQuantumDataHubNodeStep1(Player hacan, Game game, ButtonInteractionEvent event) {
        ...
        MessageHelper.sendMessageToChannelWithButtons(
                hacan.getCorrectChannel(),
                hacan.getRepresentationUnfogged() + " choose who you wish to swap strategy cards with.",
                buttons);
        event.getMessage().delete().queue();
    }
    ...
    public static void resolveSwapSC(Player player1, Game game, ButtonInteractionEvent event, String buttonID) {
        ...
        String sb =
                player1.getRepresentation() + " swapped strategy card with " + player2.getRepresentation() + "\n" + "> "
                        + player2.getRepresentationNoPing() + CardEmojis.getSCFrontFromInteger(player2SC)
                        + " :arrow_right: " + CardEmojis.getSCFrontFromInteger(player1SC) + "\n" + "> "
                        + player1.getRepresentationNoPing() + CardEmojis.getSCFrontFromInteger(player1SC)
                        + " :arrow_right: " + CardEmojis.getSCFrontFromInteger(player2SC) + "\n";
    ...
    public static void resolveHacanMechTradeStepTwo(
        ...
        if (game.isFowMode() || !game.isNewTransactionMethod()) {
            if (game.isFowMode()) {
                MessageHelper.sendMessageToChannelWithButtons(
                        hacan.getPrivateChannel(), ident + " Use Buttons To Complete Transaction", goAgainButtons);
                ...
                if (blockader.unitBelongsToPlayer(unitKey)) {
                    ...
                    MessageHelper.sendMessageToChannel(
                            ...
                            cabal.getRepresentationUnfogged() + " released " + amount + " "
                                    + blockader.getFactionEmojiOrColor() + " " + unit + " from prison due to blockade");
                    if (cabal != blockader) {
                        MessageHelper.sendMessageToChannel(
    ...
    public static void replacePDSWithFS(String buttonID, ButtonInteractionEvent event, Game game, Player player) {
        ...
        AddUnitService.addUnits(
                event, game.getTile(AliasHandler.resolveTile(planet)), game, player.getColor(), "flagship");
    ...
    public static void firstStepOfChaos(Game game, Player p1, ButtonInteractionEvent event) {
        ...
        MessageHelper.sendMessageToChannelWithButtons(
                event.getMessageChannel(), "Select which tile you would like to Chaos Map in.", buttons);
    }
    ...
    public static boolean isCabalBlockadedByPlayer(Player player, Game game, Player cabal) {
        ...
        List<Tile> tiles =
                ButtonHelper.getTilesOfPlayersSpecificUnits(game, cabal, UnitType.CabalSpacedock, UnitType.Spacedock);
        if (tiles.isEmpty()) {
            return false;
        }
        for (Tile tile : tiles) {
    ...
    public static void resolveLetnevMech(Player player, Game game, String buttonID, ButtonInteractionEvent event) {
        ...
        options.add(Buttons.red("deleteButtons", "Done Exhausting Planets"));
        MessageHelper.sendMessageToChannelWithButtons(
                event.getMessageChannel(), player.getRepresentationUnfogged() + " pay 2r for it please", options);
        ButtonHelper.deleteTheOneButton(event);
    }
    ...
    public static List<Button> getUnitButtonsForVortex(Player player, Game game, GenericInteractionCreateEvent event) {
        List<Tile> tiles =
                ButtonHelper.getTilesOfPlayersSpecificUnits(game, player, UnitType.CabalSpacedock, UnitType.Spacedock);
        if (tiles.isEmpty()) {
    ...
    public static void bentorPNPlanet(String buttonID, ButtonInteractionEvent event, Game game) {
        ...
        MessageHelper.sendMessageToChannel(
                event.getChannel(), "Attached encryption key to " + Helper.getPlanetRepresentation(planet, game));
        event.getMessage().delete().queue();
    }
    ...
    public static void resolveCreussIFFStart(
        ...
        MessageHelper.sendMessageToChannelWithButtons(
                player.getCorrectChannel(),
                player.getFactionEmojiOrColor() + " please select the tile you would like to place the " + type
                        + " wormhole in.",
                buttons);
        event.getMessage().delete().queue();
    }
    ...
    public static void resolveSalvageOps(Player player, ButtonInteractionEvent event, String buttonID, Game game) {
        ...
        String msg = player.getRepresentation() + " used _Salvage Operations_ to gain 1 trade good (you currently have "
    ...
    public static void resolveRohDhnaIndustrious(
        ...
        MessageHelper.sendMessageToChannelWithButtons(
                event.getMessageChannel(),
                "Click the names of the planets you wish to exhaust to pay the " + MiscEmojis.Resources_6,
                buttons);
        event.getMessage().delete().queue();
    }
    ...
    public static List<Button> getRohDhnaRecycleButtons(Game game, Player player) {
        ...
        Map<UnitKey, Integer> units = game.getTileByPosition(game.getActiveSystem())
                .getUnitHolders()
                .get("space")
                .getUnits();
