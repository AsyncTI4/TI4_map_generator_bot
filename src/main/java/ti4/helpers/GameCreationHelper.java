...
public class GameCreationHelper {
	...
	private static Guild getNextAvailableServerOrdinal() {
		...
		if (index == -1) { // NOT FOUND
			BotLogger.log("`GameCreationHelper.getNextAvailableServer` WARNING: Current guild is not in the list of available overflow servers: ***" + guild.getName() + "***");
		}
	...
	private static Guild getServerWithMostCapacity() {
		List<Guild> guilds = AsyncTI4DiscordBot.serversToCreateNewGamesOn.stream()
			.filter(GameCreationHelper::serverHasRoomForNewFullCategory)
			.sorted(Comparator.comparing(GameCreationHelper::getServerCapacityForNewGames))
			.toList();
	...
	private static boolean serverHasRoomForNewRole(Guild guild) {
		...
		if (roleCount >= 250) {
			BotLogger.log("`GameCreationHelper.serverHasRoomForNewRole` Cannot create a new role. Server **"
	...
	private static boolean serverHasRoomForNewChannels(Guild guild) {
		...
		if (channelCount > (channelMax - channelsCountRequiredForNewGame)) {
			BotLogger.log("`GameCreationHelper.serverHasRoomForNewChannels` Cannot create new channels. Server **"
				+ guild.getName() + "** currently has " + channelCount + " channels.");
			return false;
		}
		return true;
	...
	public static Category createNewCategory(String categoryName) {
		...
		if (guild == null) {
			BotLogger.log("`GameCreationHelper.createNewCategory` No available servers to create a new game category");
			return null;
		}
