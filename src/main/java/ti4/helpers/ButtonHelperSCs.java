...
public class ButtonHelperSCs {
            ...
            if (game.getStoredValue(key3).contains(player2.getFaction() + "*")) {
                ...
                if (!game.isFowMode()) {
                    ...
                    if (player2.getSecretsScored().size()
                        + player2.getSecretsUnscored().size() >= player2.getMaxSOCount()) {
    ...
    public static void anarchy7Build(Game game, Player player, ButtonInteractionEvent event, String buttonID) {
        ...
        List<Button> buttons;
        buttons = Helper.getPlaceUnitButtons(event, player, game, game.getTileByPosition(pos), "anarchy7Build", "place");
    ...
    public static void warfareBuild(Game game, Player player, ButtonInteractionEvent event) {
        ...
        if (val > 0 && game.playerHasLeaderUnlockedOrAlliance(player, "cabalcommander")) {
            message += ". You also have the That Which Molds Flesh, the Vuil'raith commander, which allows you to produce 2 fighters/infantry that don't count towards PRODUCTION limit. ";
        }
        if (val > 0 && IsPlayerElectedService.isPlayerElected(game, player, "prophecy")) {
            message += "Reminder that you have _Prophecy of Ixth_ and should produce at least 2 fighters if you wish to keep it. Its removal is not automated.";
        }
        if (!game.isFowMode()) {
