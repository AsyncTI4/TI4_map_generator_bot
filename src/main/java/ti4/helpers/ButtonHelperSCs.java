...
public final class ButtonHelperSCs {
    ...
    public static void resolveConstructionPrimaryTE(Game game, Player player) {
        List<Tile> tiles = UnitQueryService.getTilesContainingPlayersUnits(game, player, UnitType.Spacedock);
    ...
    public static void resolveConstructionBuild(
        ...
        String message = player.toString() + ", use these buttons to produce.";
    ...
    private static void warfareSecondaryThundersEdge(ButtonInteractionEvent event, Game game, Player player) {
        ...
        if (!used
                && !player.hasFollowedSC(warfare.getInitiative())
    ...
    private static void sendWarfareButtons(
        ...
        String message = player.toString() + " Use the buttons to produce.";
    ...
    public static void diploRefresh2(Game game, Player player, ButtonInteractionEvent event) {
        ...
        if (scModel != null && !player.hasFollowedSC(scModel.getInitiative())) {
    ...
    public static void nekroFollowTech(Game game, Player player, ButtonInteractionEvent event) {
        ...
        MessageHelper.sendMessageToChannelWithButtons(
                player.getCardsInfoThread(), player.toString() + " Exhaust planets using this", buttons2);
    }
    ...
    public static void getPreDeclineSCButtons(Game game, Player player, ButtonInteractionEvent event, String buttonID) {
        ...
        scButtons.add(Buttons.red(
                ...
                "Don't Follow "
                        + game.getStrategyCardModelByInitiative(sc).get().getName()));
    ...
    public static void scDrawSO(Game game, Player player, ButtonInteractionEvent event) {
        ...
        for (Player player2 : players) {
            ...
            if (game.getStoredValue(key3).contains(player2.getFaction() + "*")) {
                message =
                        " wishes to draw a secret objective but has people ahead of them in speaker order who need to resolve first."
                ...
                if (!game.isFowMode()) {
                    ...
                    if (player2.getSecretsScored().size()
                                    + player2.getSecretsUnscored().size()
                            >= player2.getMaxSOCount()) {
                        message +=
    ...
    public static void refresh(Game game, Player player, ButtonInteractionEvent event) {
        ...
        if (!player.hasFollowedSC(tradeInitiative)) {
    ...
    public static void refreshAndWash(Game game, Player player, ButtonInteractionEvent event) {
        ...
        if (player.hasStrategyCard(5) && !game.isHomebrewSCMode()) {
            MessageHelper.sendMessageToChannel(
                    player.getCorrectChannel(),
                    player.getRepresentationUnfogged()
        ...
        for (Player p2 : game.getRealPlayers()) {
            if (p2.hasStrategyCard(tradeInitiative) && p2.getCommodities() > 0) {
                ...
                if (p2.getPromissoryNotesInPlayArea().contains("dark_pact")
                        && !player.getPromissoryNotesOwned().contains("dark_pact")) {
                    MessageHelper.sendMessageToChannel(
                            p2.getCorrectChannel(),
                            p2.getRepresentationUnfogged()
                ...
                if (p2.hasStrategyCard(tradeInitiative)) {
                    ButtonHelperFactionSpecific.resolveDarkPactCheck(game, player, p2, commoditiesTotal);
                }
            }
            if (p2.hasStrategyCard(tradeInitiative)) {
    ...
    private static List<Button> getAnarchy7Buttons(Game game, Player player) {
        ...
        for (Tile tile : ButtonHelper.getTilesOfUnitsWithProduction(player, game)) {
            buttons.add(Buttons.green(
                    "anarchy7Build_" + tile.getPosition(), tile.getRepresentationForButtons(game, player)));
        }
        return buttons;
    }
    ...
    private static List<Button> getLumi7Buttons(Game game, Player player) {
        ...
        for (Tile tile : ButtonHelper.getTilesOfUnitsWithProduction(player, game)) {
            buttons.add(
                    Buttons.green("lumi7Build_" + tile.getPosition(), tile.getRepresentationForButtons(game, player)));
        }
        return buttons;
    }
    ...
    private static List<Button> getAnarchy3SecondaryButtons(Game game) {
        ...
        if (!Helper.getRemainingSCs(game).contains(2)
        ...
        if (!Helper.getRemainingSCs(game).contains(4)
        ...
        if (!Helper.getRemainingSCs(game).contains(5)
        ...
        if (!Helper.getRemainingSCs(game).contains(6)
        ...
        if (!Helper.getRemainingSCs(game).contains(9)
                || (game.getScPlayed().get(9) != null && game.getScPlayed().get(9))) {
            scButtons.add(Buttons.GET_A_TECH);
        }
        if (!Helper.getRemainingSCs(game).contains(11)
                || (game.getScPlayed().get(11) != null && game.getScPlayed().get(11))) {
        ...
        for (Player p2 : game.getRealPlayers()) {
            for (String leader : p2.getLeaderIDs()) {
                if (!p2.hasUnexhaustedLeader(leader)) {
                    ...
                    buttons.add(Buttons.green(
                            "anarchy2ReadyAgent_" + leader + "_" + p2.getFaction(), "Ready " + leaderM.getName()));
                }
            }
        }
    ...
    private static List<Button> getAnarchy1PrimaryButtons(Game game) {
        ...
        if (Helper.getRemainingSCs(game).contains(3)
    ...
    public static void reverseSpeakerOrder(Game game, Player player, ButtonInteractionEvent event, String buttonID) {
        ...
        MessageHelper.sendMessageToChannel(
                player.getCorrectChannel(), player.getFactionEmoji() + " has reversed speaker order.");
    }
    ...
    public static void resolveUnexpectedAction(Player player, Game game, ButtonInteractionEvent event) {
        ...
        MessageHelper.sendMessageToChannelWithButtons(
                player.getCorrectChannel(),
                player.toString() + ", please choose which system you wish to remove your command token from.",
                buttons);
    }
    ...
    public static void construction(Game game, Player player, ButtonInteractionEvent event, String buttonID) {
        ...
        if ("facility".equalsIgnoreCase(unit)) {
            ...
            if (!player.hasStrategyCard(4) && !"te4construction".equals(scModel.getBotSCAutomationID())) {
            ...
            if ("agesmonument".equalsIgnoreCase(unit)) {
                ...
                if (UnitQueryService.countUnits(game, game.getPlayerFromColorOrFaction("neutral"), "sd", true) > 2) {
                    MessageHelper.sendMessageToChannel(
                            player.getCorrectChannel(),
                            player.getRepresentationUnfogged()
                ...
                List<Button> buttons = PlanetButtonService.buttonsForUsablePlanets(
                        player,
                        game,
                        location -> !location.planet().isHomePlanet(game)
                                && !location.planet().isSpaceStation(),
                        ButtonStyle.SUCCESS,
                        "placeAgesMonument_");
                MessageHelper.sendMessageToEventChannelWithEphemeralButtons(event, message, buttons);
            } else {
    ...
    public static void placeAgesMonument(String buttonID, ButtonInteractionEvent event, Game game, Player player) {
        ...
        AddUnitService.addUnits(
                event, game.getTileContainingPlanet(planet), game, game.getNeutralColor(), "1 sd " + planet);
        MessageHelper.sendMessageToChannel(
                player.getCorrectChannel(),
    ...
    public static void startFacilityPlacement(Game game, Player player, ButtonInteractionEvent event, String buttonID) {
        ...
        for (String planet : planets) {
            buttons.add(Buttons.green(
                    "addFacility_" + planet + "_" + facility, Helper.getPlanetRepresentation(planet, game)));
        }
        String message =
                player.getRepresentationUnfogged() + ", please choose the planet you wish to put your facility on.";
        MessageHelper.sendMessageToEventChannelWithEphemeralButtons(event, message, buttons);
    }
    ...
    private static List<String> findUsablePlanetsForFacility(Game game, Player player, String facility) {
        ...
        for (String planet : player.getPlanetsAllianceMode()) {
            Planet uH = game.getPlanet(planet);
            if (uH != null) {
                if (uH.hasType("industrial") && !facility.contains("research")) {
                    planets.add(planet);
                } else if (uH.hasType("cultural")
                        && !facility.contains("research")
                        && !facility.contains("corefactory")) {
                    planets.add(planet);
                } else if (uH.hasType("hazardous")
                        && !facility.contains("research")
                    ...
                    planets.add(planet);
                } else if (uH.isLegendary()
                        && !facility.contains("research")
                        && !facility.contains("embassy")
                        && !facility.contains("core")
                        && !facility.contains("logistics")) {
                    planets.add(planet);
                }
            }
    ...
    public static void addFacility(Game game, Player player, String buttonID, GenericInteractionCreateEvent event) {
        ...
        Planet plan = game.getPlanet(planet);
        String message = player.toString() + " added a " + facility.replace("facility", "") + " facility";
        if (plan != null) {
            if (!facility.contains("embassy")) {
                plan.addToken("attachment_" + facility + ".png");
            } else {
                int embassy = getNearbyEmbassyCount(game, game.getTileContainingPlanet(planet), player);
                plan.addToken("attachment_" + facility + (embassy + 1) + ".png");
                updateEmbassies(game, player, game.getTileContainingPlanet(planet));
            }
            message += " to " + Helper.getPlanetRepresentation(planet, game);
            if (facility.contains("logistics")) {
                message +=
                        ". Their commodity value has been increased by 1 (note, the bot does not keep track of nearby structures for the purposes of this effect; players will have to monitor this)";
            }
        }
        message += ".";
        ...
        if (event instanceof ButtonInteractionEvent bEvent) {
            ...
            if (!player.hasStrategyCard(4) && !buttonID.contains("_dont")) {
    ...
    public static void anarchy10PeekStart(Game game, Player player, ButtonInteractionEvent event, String buttonID) {
        ...
        MessageHelper.sendMessageToChannelWithButtons(
                player.getCardsInfoThread(),
                player.toString() + ", please choose which objective you wish to peek at. "
                        + "They are listed in the order that they would normally be revealed.",
                buttons);
    }
    ...
    public static void anarchy10PeekAt(Game game, Player player, ButtonInteractionEvent event, String buttonID) {
        ...
        MessageHelper.sendMessageToChannelWithButtons(
                player.getCardsInfoThread(),
                player.toString() + ", please choose whether to reveal the objective or not.",
                buttons);
    }
    ...
    public static void leadershipButtons(ButtonInteractionEvent event, Game game, Player player, String buttonID) {
        ...
        if (!game.getStoredValue("ledSpend" + player.getFaction()).isEmpty()
                && !player.getSpentThingsThisWindow().isEmpty()) {
            unfinished = true;
        ...
        if (!player.hasFollowedSC(leadershipInitiative)) {
                ...
                for (Player p2 : game.getPlayers().values()) {
                    for (Integer sc2 : p2.getSCs()) {
                        if (sc2 == scNum) {
                            ...
                            MessageHelper.sendMessageToChannelWithButtons(
                                    channel,
                                    player.getRepresentationUnfogged() + " Use buttons to remove a command token.",
                                    buttonsToRemoveCC);
                        }
                    }
                }
    ...
    public static void scFollow(Game game, Player player, @NotNull ButtonInteractionEvent event, String buttonID) {
        ...
        if (player != null && player.hasStrategyCard(scNum)) {
            String message = player.toString()
        ...
        boolean used = addUsedSCPlayer(messageID, game, player);
        if (!used && !player.hasFollowedSC(scNum) && game.getPlayedSCs().contains(scNum)) {
    ...
    public static void lumiACDraw(Game game, Player player, ButtonInteractionEvent event) {
        ...
        if (!used
                && scModel != null
                && scModel.usesAutomationForSCID("luminous2")
                && !player.hasFollowedSC(scModel.getInitiative())
                && game.getPlayedSCs().contains(scModel.getInitiative())) {
            int scNum = scModel.getInitiative();
            player.addFollowedSC(scNum, event);
