...
public class CombatTempModHelper {
    ...
    public static void InitializeNewTempMods(Player player, TileModel tile, UnitHolder holder) {
        ...
        unusedMods = unusedMods.stream()
                .filter(mod -> mod.getUseInTurn() == player.getNumberTurns())
                .toList();
    ...
    public static List<NamedCombatModifierModel> BuildCurrentRoundTempNamedModifiers(
        ...
        currentRoundResults = currentRoundResults.stream()
                ...
                .filter(mod -> mod.getModifier().getForCombatAbility().equals(rollType.toString()))
                .toList();

        return currentRoundResults;
    }

    public static TemporaryCombatModifierModel getPossibleTempModifier(
            String relatedType, String relatedID, int currentTurnCount) {
