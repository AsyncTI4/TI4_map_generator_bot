...
public class TransactionHelper {
            ...
            for (String item : transactionItems) {
                if (item.contains("sending" + sender.getFaction())
                    ...
                    switch (thingToTransact) {
                        ...
                        case "Planets" -> ButtonHelperFactionSpecific.resolveHacanMechTradeStepOne(
                        ...
                        case "dmz" -> ButtonHelper.resolveDMZTrade(
                                sender, game, event, "send_" + furtherDetail + "_" + receiver.getFaction());
                        default -> resolveSpecificTransButtonPress(game, sender, spoofedButtonID, event, false);
                    }
                }
        ...
        for (Player player : players) {
            ...
            for (String item : transactionItems) {
                ...
                switch (thingToTransact) {
                    ...
                    case "shipOrders" -> trans.append(
                                    Mapper.getRelic(furtherDetail).getName())
                            .append(FactionEmojis.axis);
                    case "starCharts" -> trans.append(
                                    Mapper.getRelic(furtherDetail).getName())
                            .append(SourceEmojis.DiscordantStars);
                    case "ACs" -> {
                        switch (furtherDetail) {
                            case "generic" -> trans.append(amountToTransact)
                                    .append(" ")
                                    .append(CardEmojis.ActionCard)
                                    .append(" to be specified verbally");
                            default -> {
                    ...
                    case "Frags" -> trans.append(
                            ExploreEmojis.getFragEmoji(furtherDetail).toString().repeat(amountToTransact));
                    case "Planets", "AlliancePlanets", "dmz" -> trans.append(
                            Helper.getPlanetRepresentationPlusEmojiPlusResourceInfluence(furtherDetail, game));
                    case "action" -> trans.append("An in-game ")
                            .append(furtherDetail)
                            .append(" action");
    ...
    public static void transaction(Player player, Game game) {
        ...
        String message =
                player.getRepresentation() + " Use the buttons to select which player you wish to transact with";
        ...
        switch (thingToTrans) {
            ...
            case "PNs" -> {
                if (requesting) {
                    ...
                    message += p1.getRepresentation(true, false) + " Click the promissory note you wish to "
                            + requestOrOffer + ".";
    ...
    public static void sendOffer(Game game, Player player, String buttonID, ButtonInteractionEvent event) {
        ...
        if (game.getTableTalkChannel() != null) {
            ...
            if (!sentMeme) {
                String offerMessage =
                        "Trade offer from " + player.getRepresentationNoPing() + " to " + p2.getRepresentationNoPing()
        ...
        switch (thingToTrans) {
            ...
            case "PNs" -> {
                ...
                MessageHelper.sendMessageToChannel(
                        ...
                        "Reminder that, unlike other things,"
    ...
    public static void checkTransactionLegality(Game game, Player player, Player player2) {
        ...
        if (player.hasAbility("policy_the_people_control") && !"action".equalsIgnoreCase(game.getPhaseOfGame())) {
            sb.append("cannot transact during the agenda phase due to the ")
                    .append(FactionEmojis.olradin)
                    .append("Control policy");
