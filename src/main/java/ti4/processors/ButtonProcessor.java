    ...
    public static void resolveButtonInteractionEvent(ButtonContext context) {
        ...
        if (false) {
            ...
            switch (buttonID) { // TODO Convert all switch case to use @ButtonHandler
                ...
                case "gain1tgFromLetnevCommander" -> UnfiledButtonHandlers.gain1tgFromLetnevCommander(
                        event, player, game, mainGameChannel);
                case "gain1tgFromMuaatCommander" -> UnfiledButtonHandlers.gain1tgFromMuaatCommander(
                        event, player, game, mainGameChannel);
                ...
                case "lastMinuteDeliberation" -> UnfiledButtonHandlers.lastMinuteDeliberation(
                        event, player, game, actionsChannel);
                case "declinePDS" -> MessageHelper.sendMessageToChannel(
                        event.getMessageChannel(),
                        player.getRepresentationNoPing() + " officially declines to fire SPACE CANNON.");
