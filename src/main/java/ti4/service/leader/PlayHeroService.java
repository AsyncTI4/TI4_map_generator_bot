    ...
    public static void playHero(GenericInteractionCreateEvent event, Game game, Player player, Leader playerLeader) {
        ...
        if ("letnevhero".equals(playerLeader.getId())
            ...
            if ("zealotshero".equals(playerLeader.getId())) {
                MessageHelper.sendMessageToChannelWithButton(
                        ...
                        player.getRepresentation()
                                + ", please use the button to get your first non-faction technology.",
                        Buttons.GET_A_FREE_TECH);
                MessageHelper.sendMessageToChannelWithButton(
                        event.getMessageChannel(),
                        player.getRepresentation()
                                + ", please use the button to get your second non-faction technology.",
                        Buttons.GET_A_FREE_TECH);
                MessageHelper.sendMessageToChannelWithButton(
                        event.getMessageChannel(),
                        player.getRepresentation()
                                + ", please use the button to get your third non-faction technology.",
                        Buttons.GET_A_FREE_TECH);
            }
        } else {
            boolean purged = true;
            ...
            String leaderName = playerLeaderModel == null
                    ...
                    : playerLeaderModel.getName() + ", the " + playerLeaderModel.getFaction() + " hero,";
        ...
        switch (playerLeader.getId()) {
            case "kollecchero" -> RelicHelper.drawWithAdvantage(
            ...
            case "ghotihero" -> MessageHelper.sendMessageToChannelWithButtons(
                    event.getMessageChannel(),
            ...
            case "mortheushero" -> MessageHelper.sendMessageToChannelWithButtons(
                    event.getMessageChannel(),
            ...
            case "cymiaehero" -> {
                ...
                MessageHelper.sendMessageToChannelWithButtons(
                        player.getCorrectChannel(),
                        player.getRepresentation() + " choose whether to resolve **Autonetic Memory** or not.",
                        buttons);
            }
            case "lizhohero" -> MessageHelper.sendMessageToChannelWithButton(
                    event.getMessageChannel(),
            ...
            case "edynhero" -> {
                int size = ButtonHelper.getTilesOfPlayersSpecificUnits(game, player, Units.UnitType.Mech)
                        .size();
                MessageHelper.sendMessageToChannel(
                        player.getCorrectChannel(),
            ...
            case "vaylerianhero" -> {
                if (!game.isNaaluAgent()) {
                    ...
                    CommandCounterHelper.addCC(
                            event, player.getColor(), game.getTileByPosition(game.getActiveSystem()));
        ...
        TemporaryCombatModifierModel posssibleCombatMod = CombatTempModHelper.getPossibleTempModifier(
                Constants.LEADER, playerLeader.getId(), player.getNumberTurns());
