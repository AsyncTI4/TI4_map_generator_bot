...
public class CreateGameService {
    ...
    private static Guild getNextAvailableServerOrdinal() {
        ...
        Guild guild = AsyncTI4DiscordBot.jda.getGuildById(GlobalSettings.getSetting(
                GlobalSettings.ImplementedSettings.GUILD_ID_FOR_NEW_GAME_CATEGORIES.toString(),
                String.class,
        ...
        if (index == -1) { // NOT FOUND
            BotLogger.log(
                    "`CreateGameService.getNextAvailableServer` WARNING: Current guild is not in the list of available overflow servers: ***"
                            + guild.getName() + "***");
        }
        ...
        for (int i = index + 1; i < guilds.size(); i++) {
            ...
            if (serverHasRoomForNewFullCategory(guild)) {
                GlobalSettings.setSetting(
                        GlobalSettings.ImplementedSettings.GUILD_ID_FOR_NEW_GAME_CATEGORIES, guild.getId());
                return guild;
            }
        }
