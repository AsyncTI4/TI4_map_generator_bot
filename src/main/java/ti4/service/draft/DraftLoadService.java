...
public class DraftLoadService {
    ...
    public DraftManager loadDraftManager(Game game, String draftDataJoined) {
        ...
        for (String data : draftData) {
            if (data.startsWith(playersKey)) {
                ...
                String[] playerIds = playerIdsStr.split(DraftSaveService.DATA_SEPARATOR_QUOTED);
        ...
        for (String data : draftData) {
            if (data.startsWith(playerChoiceKey)) {
                String[] tokens =
                        data.substring(playerChoiceKey.length()).split(DraftSaveService.DATA_SEPARATOR_QUOTED, 3);
                ...
                if (orchestrator != null) {
                    String[] tokens = data.substring(playerOrchestratorStateKey.length())
                            .split(DraftSaveService.DATA_SEPARATOR_QUOTED, 2);
    ...
    private DraftOrchestrator loadOrchestrator(String data) {
        String[] orchestratorTokens = data.split(DraftSaveService.DATA_SEPARATOR_QUOTED, 2);
    ...
    private Draftable loadDraftable(String data) {
        String[] draftableTokens = data.split(DraftSaveService.DATA_SEPARATOR_QUOTED, 2);
