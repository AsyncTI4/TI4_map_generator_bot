...
public final class NavigationRelaysButton implements MoveAbilityButton {
    public boolean enabled(MoveContext ctx) {
        return ctx.player.hasTechReady("dsuydab");
    }

    public List<Button> build(MoveContext ctx) {
        return List.of(Buttons.gray("exhaustTech_dsuydab", "Exhaust Navigation Relays", FactionEmojis.uydai));
    }
}
