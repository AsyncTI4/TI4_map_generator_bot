...
public class ApplicationEmojiService {
    ...
    public static void uploadNewEmojis() {
        ...
        List<EmojiFileData> newEmojis = emojiFiles.values().stream()
            ...
            .toList();
        BotLogger.info("Uploading " + newEmojis.size() + " new emojis...", true);
    ...
    public static void reuploadStaleEmojis() {
        ...
        List<EmojiFileData> staleEmojis = emojiFiles.values().stream()
            ...
            .toList();
        BotLogger.info("Re-uploading " + staleEmojis.size() + " stale emojis...", true);
    ...
    public static void deleteHangingEmojis() {
        ...
        List<CachedEmoji> hangingEmojis = emojis.values().stream()
            ...
            .toList();
        BotLogger.info("Deleting " + hangingEmojis.size() + " hanging emojis...", true);
    ...
    public static void reportMissingEnums() {
        ...
        if (!missingEnums.isEmpty()) {
            BotLogger.error("Missing " + missingEnums.size() + " Emoji enums: " + missingEnums, true);
        }
        Set<String> missingFiles = SetUtils.difference(emojiEnumNames, emojiFileNames);
        if (!missingFiles.isEmpty()) {
            BotLogger.error("Missing " + missingFiles.size() + " Emoji files: " + missingFiles, true);
        }
    }
    ...
    private static void initFileData() {
        ...
        try {
            ...
            BotLogger.error("Unknown exception initializing emojis:", e, true);
        }
    }
    ...
    private static ApplicationEmoji createAppEmoji(EmojiFileData emoji) {
        try {
            ...
            BotLogger.error("Failed to upload emoji file: " + emoji.getName(), e, true);
            return null;
        }
    }
    ...
    private static boolean deleteAppEmoji(CachedEmoji emoji) {
        try {
            ...
            BotLogger.error("Failed to delete emoji file: " + emoji.getName(), e, true);
            return false;
        }
    }
    ...
    private static ApplicationEmoji reuploadAppEmoji(EmojiFileData file) {
        ...
        try {
            ...
            BotLogger.error(Constants.jazzPing() + " Failed to upload emoji file: " + name, e, true);
            return null;
        }
    }
    ...
    private static boolean reuploadAppEmojis(List<EmojiFileData> toReupload) {
        ...
        try {
            ...
            BotLogger.error(Constants.jazzPing() + " Failed to upload emoji files: ", e, true);
            return false;
        }
    }
    ...
    private static void resetCacheFromDiscord() {
        ...
        BotLogger.info("> - Discord has " + appEmojis.size() + " emojis.", false);
        emojis.clear();
    ...
    private static void pushEmojiListToCache(boolean isHealthy) {
        if (spoofing) return;
        if (!isHealthy) {
            BotLogger.warning(Constants.jazzPing() + " - Uploading failed, reinitializing cache from Discord.", true);
