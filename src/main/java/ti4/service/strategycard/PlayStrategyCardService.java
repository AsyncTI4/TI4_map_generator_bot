...
public class PlayStrategyCardService {
    ...
    public static void playSC(
        ...
        if (game.getPlayedSCs().contains(scToPlay) && !winnuHero) {
            MessageHelper.sendMessageToChannel(
                    event.getMessageChannel(), "**" + stratCardName + "** has already been played previously.");
            return;
        }
        ...
        if (!winnuHero && !isOverrule && game.getStoredValue("Coup").contains("_" + scToPlay)) {
            for (Player p2 : game.getRealPlayers()) {
                if (game.getStoredValue("Coup").contains(p2.getFaction())
                    ...
                    String message =
                            "Use buttons to end turn, or, if _Coup D'etat_ is Sabo'd, play **" + stratCardName + "**.";
                ...
                if (!game.isTwilightsFallMode() && !p2.getPromissoryNotes().containsKey(p2.getColor() + "_ta")) {
                    ...
                    for (Player p3 : game.getRealPlayers()) {
                        ...
                        if (p3.getPromissoryNotes().containsKey(p2.getColor() + "_ta")) {
                            ...
                            if (p3 == player) {
                                message3 +=
                                        "If you force that player to replenish commodities, then you will be prompted to play the _Trade Agreement_.";
                            } else {
                ...
                if (player3.hasUnexhaustedLeader("mahactagent")) {
                    ...
                    MessageHelper.sendMessageToChannelWithButtons(
                            ...
                            player3.getRepresentationUnfogged() + " You may follow **" + stratCardName + "** with "
                                    + (player3.hasUnexhaustedLeader("yssarilagent") ? "Clever Clever " : "")
        ...
        conclusionButtons.add(Buttons.red(player.factionButtonChecker() + "doAnotherAction", "Do Another Action"));

        if (game.isTwilightsFallMode() && (scToPlay == 2 || scToPlay == 6 || scToPlay == 7)) {
            conclusionButtons.add(Buttons.blue(
                    player.factionButtonChecker() + "endTurnWhenSpliceEnds_" + scToPlay, "End Turn When Splice Ends"));
        } else {
            conclusionButtons.add(Buttons.red(
                    player.factionButtonChecker() + "endTurnWhenAllReactedTo_" + scToPlay,
        ...
        if (!game.isHomebrewSCMode()
            ...
            graceButtons.add(
                    Buttons.green(player.factionButtonChecker() + "resolveGrace_" + scToPlay, "Resolve Grace Ability"));
                ...
                if (!player2.getPromissoryNotes().isEmpty()) {
                    for (String pn : player2.getPromissoryNotes().keySet()) {
                        if (!player2.ownsPromissoryNote("acq") && "acq".equalsIgnoreCase(pn)) {
                            ...
                            MessageHelper.sendMessageToChannelWithButtons(
                                    player2.getCardsInfoThread(), acqMessage, buttons);
                        }
                    }
                }
            }
    ...
    private static void handleScMessageResponse(
        ...
        GameMessageManager.replaceStrategyCardMessage(
        ...
        if (game.isQueueSO()) {
            for (Player player : players) {
                if (player.getSoScored() + player.getSo() < player.getMaxSOCount()
                        ...
                        || (player == imperialHolder
                                && player.controlsMecatol(true)
    ...
    private static List<Button> getLeadershipButtons(int sc) {
        Button leadershipGenerateCCButtons =
    ...
    private static List<Button> getTF2Buttons(int sc, Player player) {
        Button start = Buttons.red(player.factionButtonChecker() + "startSplice_2", "Start Splice");
        Button refreshButton = Buttons.green(player.factionButtonChecker() + "diploRefresh2", "Ready 2 Planets");
    ...
    private static List<Button> getTF6Buttons(int sc, Player player) {
        Button start = Buttons.red(player.factionButtonChecker() + "startSplice_6", "Start Splice");
        Button refreshButton =
                Buttons.green(player.factionButtonChecker() + "primaryOfAnarchy7", "Resolve PRODUCTION In A System");
    ...
    private static List<Button> getTF7Buttons(int sc, Player player) {
        Button start = Buttons.red(player.factionButtonChecker() + "startSplice_7", "Start Splice");
        Button refreshButton = Buttons.green(
                player.factionButtonChecker() + "addMagusSpliceCard", "Spend 3R And 3I To Add A Splice Card");
    ...
    private static List<Button> getLuminous1Buttons(int sc) {
        Button followButton = Buttons.green("sc_follow_" + sc, "Spend A Strategy Token");
        Button diploSystemButton = Buttons.gray("anarchy2secondary", "Ready A Relic, Technology, Or Agent.");
        Button refreshButton = Buttons.green("diploRefresh2", "Ready Planets");
    ...
    private static List<Button> getLuminous9Buttons(int sc, Player player) {
        Button followButton = Buttons.green("sc_follow_" + sc, "Spend A Strategy Token");
        Button scoreAnObjective =
                Buttons.gray(player.factionButtonChecker() + "scoreAnObjective", "Score A Public", CardEmojis.Public1);
            ...
            if (!player.isSpeaker() || game.isFowMode()) {
                ...
                if (Mapper.isValidFaction(faction)) {
                    Button button = Buttons.gray(
                            politicsHolder.factionButtonChecker()
    ...
    private static List<Button> getLuminous7Buttons(int sc) {
        Button warfarePrimary = Buttons.blue("primaryOfWarfare", "Do Warfare Primary");
        Button warfarePrimary2 = Buttons.blue("primaryOfLumi7", "Resolve PRODUCTION in a system");
        Button followButton = Buttons.green("sc_follow_" + sc, "Spend A Strategy Token");
        Button homeBuild = Buttons.green("warfareBuild", "Build At Home");
        Button noFollowButton = Buttons.blue("sc_no_follow_" + sc, "Not Following");
        return List.of(warfarePrimary, warfarePrimary2, followButton, homeBuild, noFollowButton);
    }
    ...
    private static List<Button> getTF8Buttons(int sc, Player player) {
        ...
        Button noFollowButton = Buttons.blue("sc_no_follow_" + sc, "Not Following");
        Button drawSo = Buttons.gray("sc_draw_so", "Draw Secret Objective", CardEmojis.SecretObjective);
        Button drawP = Buttons.gray("drawParadigm", "Draw Paradigm");
        Button scoreImperial =
                Buttons.gray(player.factionButtonChecker() + "score_imperial", "Score Imperial", PlanetEmojis.Mecatol);
        Button scoreAnObjective =
                Buttons.gray(player.factionButtonChecker() + "scoreAnObjective", "Score A Public", CardEmojis.Public1);
