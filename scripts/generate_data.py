...
CONFIGS = {
    ...
    "explores": {
        "input_folder": "src/main/resources/data/explores",
        "output_file": "explorations.ts",
...
def remove_blacklisted_keys(
    data: Dict[str, Any], blacklist: List[str]
) -> Dict[str, Any]:
...
def write_typescript_file(data: List[Dict[str, Any]], config: Dict[str, Any], output_dir: Path):
    ...
    try:
        with open(output_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f'export const {config["export_name"]}: {config["type_import"]}[] = '
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate TypeScript files from JSON data"
    )
    parser.add_argument(
        "data_type",
