...
RUN mvn --batch-mode --no-transfer-progress clean package && \
    ...
    cp $(pwd)/target/TI4_map_generator_discord_bot-1.0-SNAPSHOT.jar tibot.jar
ARG DISCORD_BOT_KEY
ARG DISCORD_USER
ARG DISCORD_SERVER
ARG DISCORD_SERVER2
...
# ENTRYPOINT ["sh", "-lc", "exec java -XX:MaxRAMPercentage=90.0 -XX:InitialRAMPercentage=30.0 -jar tibot.jar \"$BOT_KEY\" \"$USER\" \"$SERVER\""]
